...
`panda_link2` ships both stages (152-vertex hull, 46.83 mm → 0.00 mm).

Everything else keeps the primitive path unchanged: attached payloads,
self-collision, world-capsule obstacles, capsule-lowered robots, and every link
that declares no tight geometry. Hazard-log Entry 012's lockstep is **not
engaged** — the tight geometry is read at exactly one call site, and

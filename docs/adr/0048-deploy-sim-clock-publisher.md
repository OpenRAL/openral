...
`openral deploy sim` resolves `clock_origin` against the scene's backend:

- backend exposes sim-time (`sim_time_ns() is not None`) → `clock_origin=simulation`;
- backend returns `None` (sidecars before the protocol bump; clock-less gym envs) →

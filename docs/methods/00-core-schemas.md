...
- `class ComputeSpec(BaseModel)` — Runtime compute/accelerator specification for the host running OpenRAL. Populated by `openral_detect._enrich_compute` from GPU probe results; attached to `RobotDescription.compute`. (L538)
  fields: `onboard_compute_tops, onboard_memory_gb, gpu_vram_gb, cuda_compute_capability, cuda_toolkit_version, tensorrt_version, gpu_supported_runtimes, gpu_supported_dtypes, nvmm_available`. GPU/runtime fields moved here from `RobotCapabilities` to separate physical robot capabilities from host compute properties.

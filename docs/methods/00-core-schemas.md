...
- `class DeployScene(BaseModel)` — Env-only scene for `openral deploy run`; carries `scene`, `robot_id`, `base_pose`, and `composition: SceneComposition | None` (ADR-0066 — the scene's own MJCF composer/arena, forwarded by `openral deploy sim` to the manifest-driven HAL node as `scene_composition_json` so the scene owns its environment instead of the robot manifest); no task or eval config. Rejects legacy `vla:` blocks. (L4711)

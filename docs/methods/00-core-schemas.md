...
- `class ClockOrigin(str, Enum)` — Authoritative source for OpenRAL `stamp_ns` values. `/clock` is a ROS projection, not an origin. (L57)
  `HOST_WALL, SIMULATION, HARDWARE_SYNCED`
- `class ClockEpoch(str, Enum)` — Epoch that a `stamp_ns` value is measured from. (L69)
  `UNIX, SIMULATION_ELAPSED, HARDWARE`
- `JointRole: TypeAlias = Literal[…]` — Structural classification of a `JointSpec` (ADR-0028a). (L77)
...
- `class ControlMode(str, Enum)` — Action space / control interface. (L181)
...
- `const BODY_TWIST_DIM: int = 6` — Width of a BODY_TWIST / CARTESIAN_* twist row `(vx, vy, vz, wx, wy, wz)`. Single source for the HAL packers (`openral_hal.panda_mobile`, `openral_hal.sim_attached`) + the safety supervisor that validate 6-vec twist payloads; matches `Action.body_twist` (a 6-tuple). (L234)
- `class SensorModality(str, Enum)` — Physical sensing modality. (L247)
  `RGB, DEPTH, STEREO, IR, POINT_CLOUD, LIDAR_2D, IMU, FORCE_TORQUE, JOINT_STATE, TACTILE_VISION, TACTILE_ARRAY, AUDIO, GPS, BATTERY`
- `class Hand(str, Enum)` — End-effector laterality. (L266) `LEFT, RIGHT, NA`
- `class StateRepresentation(str, Enum)` — State vector format. (L853)
  `JOINT_POSITIONS, EEF_POS_AXISANGLE, EEF_POS_EULER, EEF_POS_QUAT, EEF_POS_AXISANGLE_GRIPPER`
- `class ActionRepresentation(str, Enum)` — Action vector format. (L863)
  `JOINT_POSITIONS, JOINT_VELOCITIES, DELTA_EE_6D_PLUS_GRIPPER, DELTA_EE_6D, CARTESIAN_POSE`
- `class RSkillAction(str, Enum)` — Closed vocabulary of high-level action verbs an rSkill can perform (ADR-0022); declared on `RSkillManifest.actions` and surfaced to the reasoner LLM tool palette so it can pick a skill by what it does. (L873)
  Manipulation primitives: `PICK, PLACE, PICK_AND_PLACE, TRANSFER, GRASP, RELEASE`; articulated / contact-rich: `OPEN, CLOSE, PUSH, PULL, SLIDE, INSERT, POUR, WIPE, ROTATE`; motion: `REACH`; mobile: `NAVIGATE`; social/expressive: `WAVE, SHAKE`; generalist marker (foundation / multi-task checkpoints): `GENERALIST`; perception producer: `DETECT` (ADR-0037, for `kind: "detector"` rSkills); scene VLM: `QUERY` (ADR-0047, for `kind: "vlm"` rSkills); reward monitor: `MONITOR` (ADR-0057, for `kind: "reward"` rSkills); playbook decision procedure: `PLAN` (ADR-0071, for `kind: "playbook"` rSkills). New entries are additive.
- `class QuantizationDtype(str, Enum)` — Weight numeric format. (L2492)
  `FP32, FP16, BF16, INT8, INT4, FP4_NVFP4`
- `class QuantizationBackend(str, Enum)` — Inference backend. (L2516)
  `PYTORCH, ONNX, TENSORRT, GGUF, MLX`
- `class RSkillState(str, Enum)` — Skill lifecycle. (L2588)
  `UNCONFIGURED, INACTIVE, ACTIVE, FINALIZED, ERROR`
- `class RSkillLicensePosture(str, Enum)` — License posture (CLAUDE §7.4). (L2668)
...
- `class RSkillRuntime(str, Enum)` — Manifest runtime hint. (L2682)
  `PYTORCH, ONNX, TENSORRT, TRT_LLM, VLLM, GGUF, MLX, JAX`
- `class PhysicsBackend(str, Enum)` — Sim backend. (L5547)
...
- `class SensorBundle(BaseModel)` — Multi-modal sensor group. (L492)
  fields: `bundle_name, sensors, sync, sync_tolerance_ms`
- `class JointSpec(BaseModel)` — URDF-derived joint spec. (L511)
...
- `class EndEffectorSpec(BaseModel)` — End-effector spec. (L582)
...
- `class ComputeSpec(BaseModel)` — Compute profile for one deployment tier (edge / local / cloud). Populated by `openral_detect._enrich_compute` from GPU probe results; attached to `RobotDescription.compute_edge`, `compute_local`, or `compute_cloud`. (L642)
...
- `class RobotCapabilities(BaseModel)` — Physical capability flags for skill compatibility. (L735)
...
- `class SafetyEnvelope(BaseModel)` — Constraints enforced by C++ safety kernel. (L783)
...
- `class ObservationSpec(BaseModel)` — VLA observation config. (L931)
  fields: `state_key, state_shape, state_representation, image_flip_180`
- `class ActionSpec(BaseModel)` — VLA action config. (L948)
...
- `class RobotDescription(BaseModel)` — Top-level robot manifest, one per robot. (L1464)
...
- `class TopCameraDefaults(BaseModel)` — Default placement for the scene-level "top" / "base" camera consumed by sim backends that render an overview camera. (L1196)
...
- `class SceneDefaults(BaseModel)` — Per-robot scene rendering defaults consulted when the scene YAML does not override them. Fields: `top_camera: TopCameraDefaults | None`, `composition: SceneComposition | None`. (L1261)
...
- `class JointState(BaseModel)` — Real-time joint state snapshot. (L1888)
  fields: `name, position, velocity, effort, stamp_ns`
- `class Pose6D(BaseModel)` — 6D pose (position + xyzw quaternion). (L1906)
  fields: `xyz, quat_xyzw, frame_id`
- `class DetectedObject(BaseModel)` — Object detection. (L1920)
...
- `class WorldState(BaseModel)` — Snapshot consumed by Reasoner and Skills. (L2104)
...
- `MemorySection: TypeAlias = Literal[...]` — the five fixed sections of the self-maintained `MEMORY.md` core (ADR-0071 §3): `home_map`, `preferences`, `lessons`, `object_locations`, `open_tasks`.
- `class MemoryWriteTool` — **write** (ADR-0071 §3); the reasoner's first write-capable variant; `tool="memory_write"`; fields `op` (`add`/`update`/`supersede`/`delete`), `section: MemorySection`, `content` (required unless `delete` — validated), `importance` (0–1, default 0.5), `target` (required for `update`/`supersede`/`delete` — validated). Edits the advisory `MEMORY.md` via an explicit op (Mem0 ADD/UPDATE/DELETE + Zep supersession); writes the memory file only — no actuation authority.
- `class MemorySearchTool` — **read-only** (ADR-0071 §3); `tool="memory_search"`; fields `query` (min_length=1), `section: MemorySection | None`, `limit` (1–100, default 5). Pages archived entries (evicted from the bounded core) back in. No actuation.
- `ReasonerToolCall: TypeAlias` — Discriminated union over the ten variants above (four actuation/effect + four ADR-0039/0043/0047 read-only query + the two ADR-0071 memory tools, one write + one read). Module docstring shows the encode/decode pattern.

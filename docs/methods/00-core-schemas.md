...
- `class StateRepresentation(str, Enum)` — State vector format. (L749)
  `JOINT_POSITIONS, EEF_POS_AXISANGLE, EEF_POS_EULER, EEF_POS_QUAT, EEF_POS_AXISANGLE_GRIPPER`
- `class ActionRepresentation(str, Enum)` — Action vector format. (L759)
  `JOINT_POSITIONS, JOINT_VELOCITIES, DELTA_EE_6D_PLUS_GRIPPER, DELTA_EE_6D, CARTESIAN_POSE`
- `class RSkillAction(str, Enum)` — Closed vocabulary of high-level action verbs an rSkill can perform (ADR-0022); declared on `RSkillManifest.actions` and surfaced to the reasoner LLM tool palette so it can pick a skill by what it does. (L769)
...
- `class QuantizationDtype(str, Enum)` — Weight numeric format. (L2381)
  `FP32, FP16, BF16, INT8, INT4, FP4_NVFP4`
- `class QuantizationBackend(str, Enum)` — Inference backend. (L2405)
  `PYTORCH, ONNX, TENSORRT, GGUF, MLX`
- `class RSkillState(str, Enum)` — Skill lifecycle. (L2477)
  `UNCONFIGURED, INACTIVE, ACTIVE, FINALIZED, ERROR`
- `class RSkillLicensePosture(str, Enum)` — License posture (CLAUDE §7.4). (L2557)
...
- `class RSkillRuntime(str, Enum)` — Manifest runtime hint. (L2571)
  `PYTORCH, ONNX, TENSORRT, TRT_LLM, VLLM, GGUF, MLX, JAX`
- `class PhysicsBackend(str, Enum)` — Sim backend. (L4893)
...
- `class SensorBundle(BaseModel)` — Multi-modal sensor group. (L388)
  fields: `bundle_name, sensors, sync, sync_tolerance_ms`
- `class JointSpec(BaseModel)` — URDF-derived joint spec. (L407)
...
- `class EndEffectorSpec(BaseModel)` — End-effector spec. (L478)
...
- `class ComputeSpec(BaseModel)` — Compute profile for one deployment tier (edge / local / cloud). Populated by `openral_detect._enrich_compute` from GPU probe results; attached to `RobotDescription.compute_edge`, `compute_local`, or `compute_cloud`. (L538)
...
- `class RobotCapabilities(BaseModel)` — Physical capability flags for skill compatibility. (L631)
...
- `class SafetyEnvelope(BaseModel)` — Constraints enforced by C++ safety kernel. (L679)
...
- `class ObservationSpec(BaseModel)` — VLA observation config. (L820)
  fields: `state_key, state_shape, state_representation, image_flip_180`
- `class ActionSpec(BaseModel)` — VLA action config. (L837)
...
- `class RobotDescription(BaseModel)` — Top-level robot manifest, one per robot. (L1353)
...
- `class TopCameraDefaults(BaseModel)` — Default placement for the scene-level "top" / "base" camera consumed by sim backends that render an overview camera. (L1085)
...
- `class SceneDefaults(BaseModel)` — Per-robot scene rendering defaults consulted when the scene YAML does not override them. Fields: `top_camera: TopCameraDefaults | None`, `composition: SceneComposition | None`. (L1150)
...
- `class JointState(BaseModel)` — Real-time joint state snapshot. (L1777)
  fields: `name, position, velocity, effort, stamp_ns`
- `class Pose6D(BaseModel)` — 6D pose (position + xyzw quaternion). (L1795)
  fields: `xyz, quat_xyzw, frame_id`
- `class DetectedObject(BaseModel)` — Object detection. (L1809)
...
- `class WorldState(BaseModel)` — Snapshot consumed by Reasoner and Skills. (L1993)

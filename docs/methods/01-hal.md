...
- module const `_DOP_VERTEX_EPS = 1e-9` (L902) — slack (metres/determinant) for the 3x3 solves behind `_dop_vertices`.
- module const `_VOXEL_BACKING_MAX_LAYERS = 4` (L1193) — how many non-collidable shells one ray looks past inside a single cell before giving up.
...
- `class SimSensorBridge` — Wires and tears down sim-sensor publishers and the MuJoCo viewer on a HAL lifecycle node, gated on the robot manifest + HAL capability. `attachment_heartbeat=False` (the HAL node sets it when its vision attachment leg is on) opens no `/openral/attachment_state` publisher for a HAL without the attachment API, so one authority publishes on that latched topic. Owns `/scan` for both a live MJCF ray-cast (`SimAttachedHAL`) and a `constant_scan_no_hit_ranges` fan for the bare digital twin. (L2502) **Attachment heartbeat for every sim HAL (2026-09-22):** `_setup_attachment_state()` always creates the `/openral/attachment_state` publisher and its 0.2 s timer; only the staging path (subscriptions + the MuJoCo evidence tracker) still requires the HAL's `update_attached_objects` / `read_attached_objects` API. The deploy launch enables the kernel's attached-payload check for every sim robot with capsules, and that check fails closed on a world state whose `attachment_stamp_ns` is still 0 — so a `MujocoArmHAL` twin (no attach mechanics, zero publishers) had every JOINT chunk dropped as `attached_overflow` on the OpenArm twin. "Nothing attached, fresh" is the exact truth for such a HAL; covered by `tests/integration/test_sim_sensor_bridge_attachment_heartbeat.py`.
  ...
  - `setup() -> None` — Activates every stream the manifest + HAL support: RGB camera + `CameraInfo` publishers (advertised lazily per camera on its first real frame — see `_advertise_camera`), the sim-only idle stepper, and the passive MuJoCo viewer (always `mjCAMERA_FREE`, opening pose from `initial_viewer_camera`; a GL/DISPLAY failure warns and continues). Idempotent per activate. (L2712)
  - `_advertise_camera(name: str) -> Any` — Creates (once) and returns the `Image` + `camera_info` publisher for one camera, on its first real rendered frame — never eagerly for the manifest's full camera list, since a manifest may declare a camera the current scene can't render and an eagerly-advertised topic would be indistinguishably silent forever. A declared camera that never yields a frame gets one warning and no topic. (L2844)
  - `teardown() -> None` — Cancel timers (incl. the idle-step timer), destroy publishers, close viewer. Called from `on_deactivate` / `on_cleanup`. (L2723)

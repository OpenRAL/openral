...
- `ConvexDistance` [@dataclass(frozen=True)] — One certified signed distance with the proof behind it: `distance_m` (the **closest** end of the bracket, so a reader is never told a pair is further apart than it provably is; negative is penetration depth), `lower_m` / `upper_m`, `certified`, `uncertified_reason`, `witness_a` / `witness_b` (nearest points on both branches — for an overlapping pair the centroid of `a`'s buried face along the SAT minimum-translation axis and that point lifted onto `b`'s supporting plane, so `(witness_a - witness_b) / distance_m` is always the `b`→`a` direction; `None` only when the depth itself was not measured — #190 needs the penetrating witness for the support-contact plane), `duality_gap_m`, `method`. `as_record() -> dict[str, object]` is the JSON-safe form that goes into a stop record — a non-finite bound serialises as `None`, never `Infinity`, because the record is read back as strict JSON. (L91)
- `ConvexBody` [@dataclass(frozen=True)] — A geom as `conv(core) ⊕ ball(radius)` plus the `faces` SAT needs; `faces` is `None` when the hull topology is unavailable (a *visual* mesh, which MuJoCo compiles no hull graph for), which costs nothing until the cores actually overlap and then yields an honest "depth not measured". (L154)
- `convex_geom_distance(model, data, geom_a: int, geom_b: int, *, distmax_m: float | None = None, arc_segments: int = 256) -> ConvexDistance` — The instrument. `distmax_m` enables a **certified** window rejection (`method="beyond-window"`) — a separating-axis bound valid in every direction, so a pair it discards is *provably* outside the window, which is what makes an exact instrument affordable over a few hundred candidates. `certified` is `False`, with a reason, for a geom with no bounded convex hull (plane/heightfield/SDF, refused by name), a round-type bracket wider than 0.1 mm, a certificate that does not close, or an overlapping pair whose exact axis set is unavailable or beyond the enumeration cap. No plausible number is ever substituted for a defensible one (CLAUDE.md §1.4). (L567)

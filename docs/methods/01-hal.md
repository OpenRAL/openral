...
- `class HALLifecycleNodeBase(LifecycleNode)` — Public base class. Owns the standard `/joint_states` + `~/joint_states` publishers, the `/openral/safe_action` + `/openral/estop` subscribers, the 1 Hz `DiagnosticsHeartbeat`, the per-tick `hal.read_state` + `hal.send_action` OTel spans, the estop latch, and the full configure → activate → deactivate → cleanup → shutdown transition wiring. The formerly used `~/command` (`trajectory_msgs/JointTrajectory`) subscriber + its `_on_command` callback + the `_subscriber` field were removed; `_send_action_traced` is now driven only by `_on_safe_action`. (L382)
  ...
  - `_on_safe_action(self, msg) -> None` — `/openral/safe_action` callback. Decodes the `openral_msgs/ActionChunk` into an `openral_core.Action` and forwards through `_send_action_traced(action, source="safe_action")`. (L1007)
  - `_send_action_traced(self, action, *, source) -> None` — Forward `action` to `self._hal.send_action` inside a `hal.send_action` span. The `source` attribute disambiguates the origin on the dashboard's Commands card (kept on the span so future subscriber additions can fan in without changing the span shape). (L1083)
...
- `class SimSensorBridge` — Wire and tear down sim-sensor publishers and the MuJoCo viewer on a HAL lifecycle node. Streams are gated on the robot manifest + HAL capability. Owns `/scan` for **both** paths since issue #191 Phase 3: live MJCF ray-cast when `hal.mujoco_handles()` is bound (`SimAttachedHAL`), a `constant_scan_no_hit_ranges` fan for the bare digital twin (the node no longer publishes its own scan). (L825)
  ...
  - `_advertise_camera(name: str) -> Any` — Create (once) and return the `Image` publisher for one camera, plus its `camera_info` companion. Called from `_publish_images` the first time the backend actually hands up a frame for `name`, so the advertised topic set is exactly what this scene renders — never the manifest superset. A manifest may legitimately declare a camera the current scene cannot supply (`panda_mobile`'s synthetic `head` nav cam needs a mobile base + `OPENRAL_ROBOCASA_HEAD_CAM=1`, which `deploy` sets only when a capability-matched rSkill consumes it, so a manipulation-only palette declares the sensor and no backend renders it); advertising eagerly left a permanently silent `/openral/cameras/head/image` that a subscriber cannot distinguish from a slow stream and so waits on forever. A declared camera that never yields a frame gets one warning naming the available obs keys and no topic. (L1145)
  - `teardown() -> None` — Cancel timers (incl. the idle-step timer), destroy publishers, close viewer. Called from `on_deactivate` / `on_cleanup`. (L1024)
...
- `class VisionGateConfig` — Frozen dataclass of gate thresholds: `containment_radius_m=0.12`, `max_payload_extents_m=(0.10, 0.25, 0.25)`, `max_payload_volume_m3=0.004`, `min_depth_valid_fraction=0.30`, `min_depth_m=0.02`, `max_depth_m=1.0`, `view_ray_inflation_m=0.02`, `jaw_span_m=0.05`, `max_primitives=16`. **Every value is a chosen calibration point, not a measured constant** — the design work benchmarked latency/VRAM/mask quality and no gate threshold. The extent cap is per-axis rather than a scalar volume because only the jaw axis is hard-bounded; `max_payload_volume_m3` is a backstop. The extent and jaw-span caps are **not derivable from the manifest today** — `EndEffectorSpec` has no jaw-aperture/gripper-span field. (L130)
- `class VisionAttachmentReport` — Frozen dataclass carrying what the gates measured, for the trace: `accepted`, `rejections` (`"empty_mask"` / `"depth_validity"` / `"payload_extent"` / `"payload_volume"` / `"jaw_containment"` / `"no_valid_depth"`), `depth_valid_fraction`, `point_count`, `extents_m`, `volume_m3`, `centroid_distance_m`, `mask_score_advisory`. The last field is recorded only — never compared against a threshold. (L199)
- `backproject_masked_depth(mask, depth_m, intrinsics, *, min_depth_m, max_depth_m) -> tuple[NDArray, float]` — Back-project a mask's pixels through a metric depth frame into the camera **optical** frame (REP-103), returning `(N, 3)` points and the fraction of masked pixels that carried usable depth. An empty mask returns `((0, 3), 0.0)` — "nothing was masked" is a gate result, not an error. Raises `ROSConfigError` on mask/depth shape disagreement or intrinsics at a different resolution (rescale with `openral_core.scale_intrinsics_to` first). (L229)
- `clustered_obb_primitives(points_in_link, *, object_id, max_primitives=16, inflation_m=0.0, inflation_axis_in_link=None) -> tuple[list[AttachedCollisionPrimitive], NDArray, NDArray]` — Reduce a payload cloud to bounded oriented boxes, using the **same** reduction as the sim producer's `_clustered_box_primitives` (sort along the widest axis, `np.array_split` into `<= max_primitives` clusters, AABB each) but splitting in the cloud's PCA frame so the boxes are oriented to the object. Splitting is strictly tighter than a single box and never drops an observed point. `inflation_axis_in_link` pads each axis by its share of the view ray, compensating for the unobserved far side. Returns the primitives plus the object frame's origin and 3x3 rotation in the link frame. Clouds below 24 points stay axis-aligned rather than orienting off noise. (L315)
- `jaw_span_primitive(*, object_id, jaw_span_m) -> AttachedCollisionPrimitive` — The conservative fallback box: a cube of half-extent `jaw_span_m` centred on the TCP. Deliberately crude — its job is that a rejected mask still leaves geometry in front of the collision checker. (L405)
- `class VisionAttachmentEvidenceProducer(description, *, config=None)` — Event-driven producer (no per-frame `update`, unlike the ticked sim tracker). Resolves `attach_link` / `touch_links` from the manifest's gripper-role joints; raises `ROSConfigError` when there are none or they do not share one parent link. (L442)
  - `on_grasp(*, mask, depth_m, intrinsics, t_link_from_cam, tcp_in_link, object_id, stamp_ns, mask_score_advisory=0.0) -> tuple[AttachedCollisionObject, VisionAttachmentReport]` — Fit and gate one payload. **Always returns an attachment**: a mask failing any gate degrades to the `jaw_span_primitive` box stamped `AttachmentEvidenceKind.GRIPPER_FORCE` at low confidence, never `None` and never a silent skip. An accepted mask is stamped `AttachmentEvidenceKind.VISION_SEGMENTATION`; `mass_kg` / `center_of_mass_m` / `inertia_kg_m2` stay `None` (vision gives geometry only). `t_link_from_cam` is supplied by the caller from TF2 — this module never composes frames itself. (L505)
  - `on_release() -> list[AttachedCollisionObject]` — Detach: an empty list, which is a *complete* attachment set meaning "nothing is held", mirroring the sim producer's release path. (L496)

...
- `ConvexDistance` [@dataclass(frozen=True)] — One certified signed distance with the proof behind it: `distance_m` (the **closest** end of the bracket, so a reader is never told a pair is further apart than it provably is; negative is penetration depth), `lower_m` / `upper_m`, `certified`, `uncertified_reason`, `witness_a` / `witness_b`, `duality_gap_m`, `method`. `as_record() -> dict[str, object]` is the JSON-safe form that goes into a stop record — a non-finite bound serialises as `None`, never `Infinity`, because the record is read back as strict JSON. (L91)
- `ConvexBody` [@dataclass(frozen=True)] — A geom as `conv(core) ⊕ ball(radius)` plus the `faces` SAT needs; `faces` is `None` when the hull topology is unavailable (a *visual* mesh, which MuJoCo compiles no hull graph for), which costs nothing until the cores actually overlap and then yields an honest "depth not measured". (L147)
- `convex_geom_distance(model, data, geom_a: int, geom_b: int, *, distmax_m: float | None = None, arc_segments: int = 256) -> ConvexDistance` — The instrument. `distmax_m` enables a **certified** window rejection (`method="beyond-window"`) — a separating-axis bound valid in every direction, so a pair it discards is *provably* outside the window, which is what makes an exact instrument affordable over a few hundred candidates. `certified` is `False`, with a reason, for a geom with no bounded convex hull (plane/heightfield/SDF, refused by name), a round-type bracket wider than 0.1 mm, a certificate that does not close, or an overlapping pair whose exact axis set is unavailable or beyond the enumeration cap. No plausible number is ever substituted for a defensible one (CLAUDE.md §1.4). (L539)
...
- `class SimSensorBridge` — Wire and tear down sim-sensor publishers and the MuJoCo viewer on a HAL lifecycle node. Streams are gated on the robot manifest + HAL capability. Owns `/scan` for **both** paths since issue #191 Phase 3: live MJCF ray-cast when `hal.mujoco_handles()` is bound (`SimAttachedHAL`), a `constant_scan_no_hit_ranges` fan for the bare digital twin (the node no longer publishes its own scan). (L1743)
  ...
  - `_advertise_camera(name: str) -> Any` — Create (once) and return the `Image` publisher for one camera, plus its `camera_info` companion. Called from `_publish_images` the first time the backend actually hands up a frame for `name`, so the advertised topic set is exactly what this scene renders — never the manifest superset. A manifest may legitimately declare a camera the current scene cannot supply (`panda_mobile`'s synthetic `head` nav cam needs a mobile base + `OPENRAL_ROBOCASA_HEAD_CAM=1`, which `deploy` sets only when a capability-matched rSkill consumes it, so a manipulation-only palette declares the sensor and no backend renders it); advertising eagerly left a permanently silent `/openral/cameras/head/image` that a subscriber cannot distinguish from a slow stream and so waits on forever. A declared camera that never yields a frame gets one warning naming the available obs keys and no topic. (L2066)
  - `teardown() -> None` — Cancel timers (incl. the idle-step timer), destroy publishers, close viewer. Called from `on_deactivate` / `on_cleanup`. (L1945)

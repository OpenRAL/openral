...
- `points_from_depth_grid(depth, *, fx, fy, cx, cy) -> NDArray[np.float32]` — Back-projects an `(H, W)` metric-depth raster (optical-Z, `0.0` = no measurement) into the `(N, 3)` float32 optical-frame cloud `((col-cx)/fx·z, (row-cy)/fy·z, z)`, dropping the `0.0` pixels — the same sparse, hit-only cloud `synthesize_depth_pointcloud` returns, in the same row-major ray order. Takes the intrinsics **of the raster** (stride-scaled, i.e. what the companion `CameraInfo` advertises). This is what lets `SimSensorBridge._publish_depth_clouds` pay for one ray-cast per camera per frame instead of two: it synthesises the raster once, publishes it as the `32FC1` image, and back-projects it for octomap's `PointCloud2`. (L583)
...
- `class HALLifecycleNodeBase(LifecycleNode)` — Public base class. Owns the standard `/joint_states` + `~/joint_states` publishers, the `/openral/safe_action` + `/openral/estop` subscribers, the 1 Hz `DiagnosticsHeartbeat`, the per-tick `hal.read_state` + `hal.send_action` OTel spans, the estop latch, and the full configure → activate → deactivate → cleanup → shutdown transition wiring. The formerly used `~/command` (`trajectory_msgs/JointTrajectory`) subscriber + its `_on_command` callback + the `_subscriber` field were removed; `_send_action_traced` is now driven only by `_on_safe_action`. (L374)
  - `_create_hal(self) -> HAL` — **Subclass hook (required)**: construct and return a HAL instance. Reads ROS-parameter-driven constructor args via `self.get_parameter(...)`. (L447)
  - `_heartbeat_extra_fields(self) -> dict[str, str]` — Subclass hook (optional): extra key/values for the `/diagnostics` payload (e.g. `{"port": "/dev/ttyUSB0"}` for SO-100, `{"mjcf": "..."}` for OpenArm). Default: `{}`. (L459)
  - `on_configure_post_hal(self) -> TransitionCallbackReturn` — Subclass hook (optional): robot-specific setup after the HAL connects (e.g. opening a camera renderer on OpenArm). Default: `SUCCESS`. (L511)
  - `on_activate_post_subs(self) -> TransitionCallbackReturn` — Subclass hook (optional): robot-specific timers/publishers after the base wires its subs (e.g. the OpenArm camera-render timer). Default: `SUCCESS`. (L520)
  - `on_deactivate_pre_teardown(self) -> None` — Subclass hook (optional): stop robot-specific timers before base teardown. Default: no-op. (L528)
  - `on_cleanup_pre_disconnect(self) -> None` — Subclass hook (optional): tear down robot-specific resources (viewers, renderers) before HAL.disconnect(). Default: no-op. (L535)
  - `_publish_joint_state(self) -> None` — Timer callback. Wraps `self._hal.read_state()` in a `hal.read_state` span (identity attrs + `producer.record_joint_state`) and publishes the standard `/joint_states` + `~/joint_states` messages; when the HAL exposes `read_policy_state`, also publishes `/openral/policy_state` (`std_msgs/Float32MultiArray`) — only when the underlying `ProprioFrame` is NEW (one publish per env.step capture, never a latched republish, so the world-state aggregator's dedicated `policy_state` staleness window genuinely trips on a wedged simulator). Subclasses may override + call `super()._publish_joint_state()` to extend (OpenArm does this for viewer-sync). (L871)
  - `_on_safe_action(self, msg) -> None` — `/openral/safe_action` callback. Decodes the `openral_msgs/ActionChunk` into an `openral_core.Action` and forwards through `_send_action_traced(action, source="safe_action")`. (L966)
  - `_send_action_traced(self, action, *, source) -> None` — Forward `action` to `self._hal.send_action` inside a `hal.send_action` span. The `source` attribute disambiguates the origin on the dashboard's Commands card (kept on the span so future subscriber additions can fan in without changing the span shape). (L1042)
...
- `class SimAttachedHAL` — HAL Protocol adapter wrapping an in-process `SimRollout`. Reads live joint state via `normalized_joint_index` + `mj_name2id`; sends actions via `pack_action_for_env` (or a caller-supplied `ActionPacker`) into `env.step()`. Exposes `read_images()`, `mujoco_handles()`, `sim_time_ns()`, `base_pose`, `base_twist`, `base_pose_6dof()` for the ROS lifecycle node's camera publisher, viewer, sim-clock, and odom wiring. (L357)
  ...
  - `mujoco_handles() -> tuple[Any, Any] | None` — Forward the env's `(model, data)` MJCF handles. (L1505)
  ...
  - `estop() -> None` — Latch e-stop; subsequent `send_action` calls are dropped. (L1497)
  - `base_pose -> tuple[float, float, float]` [@property] — Current `(x, y, yaw)`: from MJCF qpos on a MuJoCo backend, else from `obs["base_pose"]` the SimRollout surfaces (Isaac kinematic base); `(0,0,0)` when the backend reports neither. Feeds the `/odom` publisher. (L1694)
  - `base_twist -> tuple[float, float, float, float, float, float]` [@property] — Last commanded body twist `(vx, vy, vz, wx, wy, wz)`. (L1747)
  ...
  - `base_pose_6dof() -> tuple[...] | None` — Full 6-DoF `(xyz, quat_xyzw)` from robocasa `raw_proprio`; falls back to `None` for non-robocasa backends. (L1757)
  - `last_action_ns -> int` [@property] — Monotonic ns of the last real action through `send_action`; `0` until the first one. The idle stepper reads it (via `should_idle_step`) to yield to an active skill. (L1620)

### `python/hal/src/openral_hal/sim_sensor_bridge.py`
_Shared sim-sensor + viewer bridge for scene-attached HAL lifecycle nodes. Republishes RGB camera frames and a live MuJoCo viewer for any manifest-driven node, and runs the sim-only idle stepper. Phase 2 adds `/scan` + depth `PointCloud2`. Depth comes from the MuJoCo ray-cast (`_publish_depth_clouds` — **one** `synthesize_depth_image` cast per camera per frame; the `PointCloud2` is that raster back-projected by `points_from_depth_grid`, not a second cast) OR, for a non-MuJoCo backend that surfaces ready `base_link` clouds in obs (the Isaac scene), `_publish_depth_clouds_from_obs` — which wraps `hal.read_depth_clouds()` into a `base_link` `PointCloud2` (no ray-cast, no per-camera optical TF); `_setup_depth` creates the publishers when either source is present. When the HAL supports attachments, the bridge subscribes the atomic TRANSIENT_LOCAL `/openral/attachment_state` snapshot and applies the same payload set to the depth mask that World State/kernel consume. It also owns the **E-stop ground-truth record** (`_setup_estop_ground_truth`): one `sim.estop_ground_truth_snapshot` line per kernel stop, carried payload or not. rclpy imported lazily._
...
- `estop_ground_truth_snapshot(model, data, *, robot_body_ids: frozenset[int], attached_body_ids: frozenset[int] = frozenset(), probe_body_ids: frozenset[int] | None = None, base_frame_body: str | None = None, joint_state: Any = None, distmax_m: float = 0.10, max_pairs: int = 32, max_calls: int = 4096) -> dict[str, object]` — Pure (no rclpy) MuJoCo ground truth for **one safety stop, carried payload or not**. Returns `stop_class` (`"attached_payload"` / `"robot_world"`), `sim_time_s`, `contacts_caveat`, `attached_bodies`, `payload_contacts`, `robot_world_contacts`, `nearest_robot_world_pairs`, `nearest_payload_world_pairs`, `nearest_payload_robot_pairs`, `probe_robot_scope`, `probe_excluded_robot_bodies`, `robot_joint_state`, `base_frame_tf`. Contact records normalise the queried side onto `_a` (MuJoCo orders a contact's geoms by id) and carry both geom + body names, both body world positions, the signed distance and the contact point. **The contact lists are not a penetration oracle** — contype/conaffinity exclusions can suppress a contact entirely (field-observed: an arm 30 mm inside a freezer door with `ncon == 0`), which is why the record repeats `contacts_caveat` and why the three `mj_geomDistance` near-miss probes are the adjudicator. Probe semantics: **robot↔world** ranked over `probe_body_ids` (the kernel-checked links from `kernel_checked_body_ids`; `None` falls back to the whole robot and is reported as `probe_robot_scope: "all_robot_bodies"`) with the *whole* robot excluded from the world side; **payload↔world** with the robot excluded; **payload↔robot-link** self-pairs against the same kernel-checked links (the kernel's own `check_attached_self_collision`). Each is bounded by a vectorised `geom_rbound` prefilter, `max_calls` exact calls in proximity order (~3.3 ms per 4096) and the closest `max_pairs` reported; unbounded geoms (planes/heightfields, `rbound == 0`) stay admitted but rank at `0.0` so a kitchen floor cannot outrank real bounding-sphere overlap. Called by `SimSensorBridge._on_estop_ground_truth`; unit-testable against a compiled `MjModel` (`tests/unit/test_sim_estop_ground_truth.py`, `tests/unit/test_sim_estop_probe_scope.py`). (`sim_sensor_bridge.py`)
...
- `candidate_chunk_digest(*, stamp_ns: int, control_mode: int, horizon: int, n_dof: int, flat: Sequence[float], cartesian_delta_scale: Sequence[float] = (), ee_name: str = "", frame_id: str = "", rskill_id: str = "", trace_id: str = "", tick_index: int = 0) -> dict[str, object]` — Pure (no rclpy) JSON-safe digest of one `openral_msgs/ActionChunk`, reshaped into `horizon` rows of `n_dof` under `ticks` (a length that disagrees is kept verbatim under `flat` + `shape_mismatch: true`, never truncated) with `control_mode` decoded through `UINT8_TO_CONTROL_MODE`. This is the reconstruction input for a **predicted-horizon** stop (`CollisionEvidence.horizon_step >= 0`), whose configuration exists nowhere in the simulator and which TF therefore cannot reproduce: chunk rows + the measured `robot_joint_state` + `horizon_step` let an offline tool re-run the kernel's own FK. Cached per chunk by `SimSensorBridge._on_candidate_action`. (`sim_sensor_bridge.py`)
- `class SimSensorBridge` — Wire and tear down sim-sensor publishers and the MuJoCo viewer on a HAL lifecycle node. Streams are gated on the robot manifest + HAL capability. Owns `/scan` for **both** paths since issue #191 Phase 3: live MJCF ray-cast when `hal.mujoco_handles()` is bound (`SimAttachedHAL`), a `constant_scan_no_hit_ranges` fan for the bare digital twin (the node no longer publishes its own scan). (L635)
  ...
  - `_advertise_camera(name: str) -> Any` — Create (once) and return the `Image` publisher for one camera, plus its `camera_info` companion. Called from `_publish_images` the first time the backend actually hands up a frame for `name`, so the advertised topic set is exactly what this scene renders — never the manifest superset. A manifest may legitimately declare a camera the current scene cannot supply (`panda_mobile`'s synthetic `head` nav cam needs a mobile base + `OPENRAL_ROBOCASA_HEAD_CAM=1`, which `deploy` sets only when a capability-matched rSkill consumes it, so a manipulation-only palette declares the sensor and no backend renders it); advertising eagerly left a permanently silent `/openral/cameras/head/image` that a subscriber cannot distinguish from a slow stream and so waits on forever. A declared camera that never yields a frame gets one warning naming the available obs keys and no topic. (L951)
  - `teardown() -> None` — Cancel timers (incl. the idle-step timer), destroy publishers, close viewer. Called from `on_deactivate` / `on_cleanup`. (L833)
  - `_setup_estop_ground_truth() / _on_estop_ground_truth(msg)` — **Diagnostics only** (nothing gates, delays, or alters actuation). Gated on live MuJoCo handles — deliberately **not** on attachment support, because the class this closes is the *pre-grasp* arm↔world stop: the previous `sim.attached_payload_estop_snapshot` early-returned whenever nothing was carried, so 3 of the 4 stops in the 2026-08-13 post-fix matrix produced zero ground truth and could not be adjudicated real-vs-false. Subscribes `/openral/estop` (fires the snapshot), `/openral/candidate_action` (a 3-deep ring of `candidate_chunk_digest`s — the chunk the kernel was checking) and `/openral/failure/safety` (the kernel's `CollisionEvidence`, carrying `horizon_step` / `link_a` / `min_distance_m`). Emits one `sim.estop_ground_truth_snapshot` JSON line per stop: `estop_ground_truth_snapshot` (scoped to `kernel_checked_body_ids`, falling back to the whole robot when the manifest declares no collision geometry) + `stop_seq` + the cached chunks + the evidence when it landed within 0.5 s. The kernel publishes the failure trigger *then* the E-stop, but cross-topic delivery order is not guaranteed; the snapshot is never delayed for it (sim state must be captured at the stop instant), so late evidence is emitted as its own `sim.estop_ground_truth_evidence` line joined by `stop_seq`.

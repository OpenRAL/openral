...
- `estop_ground_truth_snapshot(model, data, *, robot_body_ids: frozenset[int], attached_body_ids: frozenset[int] = frozenset(), base_frame_body: str | None = None, joint_state: Any = None, distmax_m: float = 0.10, max_pairs: int = 8) -> dict[str, object]` — Pure (no rclpy) MuJoCo ground truth for **one safety stop, carried payload or not**. Returns `stop_class` (`"attached_payload"` / `"robot_world"`), `sim_time_s`, `attached_bodies`, `payload_contacts`, `robot_world_contacts`, `nearest_robot_world_pairs`, `robot_joint_state`, `base_frame_tf`. Contact records normalise the queried side onto `_a` (MuJoCo orders a contact's geoms by id) and carry both geom + body names, both body world positions, the signed distance and the contact point. Because the kernel stops on a **margin**, a genuine stop usually leaves `ncon == 0` — so the near-miss probe runs `mujoco.mj_geomDistance` over the robot↔world geom pairs a vectorised `geom_rbound` prefilter admits (≤256 exact calls, closest `max_pairs` reported), which is what makes a "−15 mm" evidence line adjudicable. Called by `SimSensorBridge._on_estop_ground_truth`; unit-testable against a compiled `MjModel` (`tests/unit/test_sim_estop_ground_truth.py`). (`sim_sensor_bridge.py`)

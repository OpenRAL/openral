...
- `support_patch_withholds(patch, center, resolution) -> bool` — Does an attested patch claim this cell, and therefore forbid clearing it? The kernel's `support_contact_exempts` with `slack = 0`: inside the patch laterally (padded by the cell circumradius) and no higher above the attested plane than `half_resolution·(|n.x|+|n.y|+|n.z|) + max_penetration` — the support **half-space slab**, never the whole patch cylinder. Dropping the slack keeps the withheld set a **subset** of the kernel-exempt set for the object that attested it, so a withheld cell can never be the one that stops the robot against that object; the two scope conditions that qualify it (the kernel exempts per-object while the bridge withholds per-message, and the kernel's lifecycle node caps the attestation it accepts) are in the [package README](../../packages/openral_octomap_bridge/README.md). Deliberate mirror of the kernel predicate — see [14-duplication-watch.md](14-duplication-watch.md) item 8.
...
- `attach_transition_padding(steady_padding_m, attach_sweep_padding_m) -> double` — The clearing padding one object gets on ONE grid: the steady value plus the attach-sweep value while its window is open, so the widened sweep is never *tighter* than an ordinary frame. Non-finite or negative inputs contribute 0 — a parameter cannot narrow the clearing below what the payload's volume explains. The extra reach exists for the stale pre-attach silhouette a fitted primitive leaves just outside one circumradius (the round-5 `voxel_91633`, 22.13 mm out at 25 mm cells); see the [package README](../../packages/openral_octomap_bridge/README.md).
...
- `struct GridSpec` — Output lattice: `resolution`, `sx`/`sy`/`sz`, `box_min[3]` (the local volume's min corner in the base frame).
- `rasterize_octree_to_grid(tree, base_to_octomap, spec, base_frame) -> openral_msgs::msg::OccupancyVoxels` — Crop + rasterize a bounded local volume of a real `octomap::OcTree` into the kernel-facing dense grid: each base-frame cell centre is mapped into the octree frame by `base_to_octomap` (`lookupTransform(octomap_frame, base_frame)`) and queried. `header.frame_id = base_frame`, with `origin` in that same frame (the ADR-0095 frame contract).

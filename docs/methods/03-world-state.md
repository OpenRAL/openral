...
- `support_patch_withholds(patch, center, resolution) -> bool` — Does an attested patch claim this cell, and therefore forbid clearing it? The kernel's `support_contact_exempts` with `slack = 0`: inside the patch laterally (padded by the cell circumradius) and no higher above the attested plane than `half_resolution·(|n.x|+|n.y|+|n.z|) + max_penetration`. Dropping the slack keeps the withheld set a **subset** of the kernel-exempt set, so a withheld cell can never be the one that stops the robot. Deliberate mirror of the kernel predicate — see [14-duplication-watch.md](14-duplication-watch.md) item 8.

...
- `class ChunkedExecutor` — Owns the per-step action buffer for every chunked VLA family and optionally overlaps chunk N+1 inference with execution via a background daemon thread. Default producer is `policy.predict_action_chunk` (never `select_action`/`reset` — a prefetch that reset the shared policy queue used to trigger a racing double inference at every chunk boundary); a custom `chunk_fn(payload) -> chunk` supports adapters with autocast contexts / chunk-level decode / non-lerobot APIs (pi05, gr00t, molmoact2, openvla). Chunks may be `(B,T,D)` tensors (lerobot layout, truncated to `chunk_size`) or ready sequences of per-step actions (producer-sized, not truncated). `select_action(batch_or_fn)` accepts a plain payload or a zero-arg callable, materialised only when an inference launches (buffer pops build nothing and touch no policy). `prefetch_at=0` = plain synchronous chunk buffer (eval semantics identical to lerobot's queue); default 15 (≈500 ms cover at 30 Hz); chunks shorter than the threshold trigger right after their first pop. (L65)
  - `__init__(policy=None, *, chunk_fn=None, chunk_size=None, prefetch_at=15)` — policy OR chunk_fn+chunk_size; ValueError otherwise. (L92)
  - `start() -> None` — Mark as running (call after policy is on-device). (L160)
  - `stop() -> None` — Signal background thread, join. (L164)
  - `reset() -> None` — Clear buffer/bg state; `policy.reset()` when a policy was given. (L172)
  - `select_action(batch_or_fn) -> Any` — Next action; cold-start foreground inference / buffer pop / wait-on-prefetch. (L188)
  - private: `_pop_and_maybe_prefetch(batch)` (every pop routes through it so the trigger is branch-independent), `_produce(payload, chunk_index, kind)`, `_materialize`, `_extend_buffer`, `_launch_prefetch(batch)`
...
- `InferenceKind` — re-export of `openral_observability.InferenceKind` (`Literal["foreground", "prefetch", "single"]`), the closed value set for the `inference.kind` label; the Literal lives with the span helper that owns the label. `kind` is a TIMING axis (critical-path / background-prefetch / per-step-eval) — there is deliberately no `"chunk"`: chunk shape rides `inference.chunk_size`/`chunk_index`, and four adapters that used to pass `kind="chunk"` through the then-untyped parameter now record `foreground`. (L46)
- `resolve_device(spec: VLASpec) -> str` — `"auto"` → `"cuda:0"` / `"mps"` / `"cpu"` against real torch. (L49)
- `resolve_rskill_repo_id(weights_uri: str, *, adapter_name: str) -> str` — Validate skill reference and resolve to bare HF repo id; `adapter_name` is used in the `ROSConfigError` message. (L74)
- `resolve_rskill_repo_revision(weights_uri: str, *, adapter_name: str) -> tuple[str, str | None]` — Like `resolve_rskill_repo_id` but also returns the optional `@<sha>` revision pin (threaded into `from_pretrained`/`snapshot_download` by the sim adapters) and warns `rskill.unpinned_weights` when an `hf://` skill is unpinned (security audit 2026-06, H4). (L105)
- `apply_chunk_replay(policy, spec_extra) -> int` — Override `policy.config.n_action_steps` from `vla.extra` (default `chunk_size // 2`); used by all lerobot-style adapters (`smolvla`, `act`, `pi05`) to amortise the heavy chunk forward over multiple env steps. (L281)
- `_CUDAGRAPH_COMPILE_MODES` — `frozenset({"reduce-overhead", "max-autotune"})`; the `torch.compile` modes that may capture CUDA graphs and therefore require output cloning (static replay buffers would otherwise be overwritten under lerobot's queued action views / `ChunkedExecutor` pre-fetch). (L340)
- `_has_bnb_quantized_modules(policy) -> bool` — True when any submodule's class comes from `bitsandbytes` (`Linear4bit` / `Linear8bitLt` rewrites from `openral_sim._quantization`). Class-module-path check; never imports bnb. (L352)
- `_clone_chunk_output(out, torch) -> Any` — Recursively `.clone()` every tensor in a chunk forward's output (tensor / tuple / list / dict; non-tensor leaves pass through) so downstream holders own their storage, detached from CUDA-graph static buffers. (L366)
- `maybe_compile_chunk_forward(policy, spec_extra, device, torch, *, method_name="_get_action_chunk") -> bool` — Best-effort `torch.compile` of the chunk forward with a runtime fallback wrapper (latches into eager mode on backend errors). Skipped on CPU and when `vla.extra.compile` is falsy. Two safety gates: bitsandbytes-quantized policies are never compiled (mixed nf4/bf16 graphs trip dtype-mismatch errors — the same reason the pi05 adapter forces `compile_model = False`; logs `vla_compile_skipped_bnb_quantized`), and under cudagraph modes (`_CUDAGRAPH_COMPILE_MODES`) every output is routed through `_clone_chunk_output` on both the compiled and eager-fallback branches. Logs `vla_compile_setup_failed` / `vla_compile_runtime_fallback` on failure. (L384)
- `run_inference(policy, batch, *, chunk_index=None, kind="single", chunk_size=None, engine=None, method="select_action", call=None) -> Tensor` — Single seam wrapping a policy inference call in `inference_span` + `torch.no_grad()`; the only place `inference.kind` / `chunk_index` / `chunk_size` / `inference.engine` / `inference.device` attributes are emitted across both eval and skill paths. `engine` defaults to `"torch"`; `device` is auto-lifted from `policy.device` when present. `method` selects the policy entry point — `"select_action"` (queue-popping single step, default) or `"predict_action_chunk"` (pure chunk producer). `call` invokes a custom callable instead of a policy method (same span, `policy` may be `None`) — how `ChunkedExecutor` instruments custom `chunk_fn` producers. (L487)
- `build_chunk_executor(spec_extra, *, policy=None, chunk_fn=None, chunk_size=None, adapter_name="policy") -> ChunkedExecutor | None` — The ONE wiring seam every chunked adapter uses: returns a STARTED executor (`None` when effective chunk size <= 1). `spec_extra["chunk_prefetch"]` (deploy-runtime setdefault; manifest `policy_extras` can pin off) only picks `prefetch_at` — truthy → 15 (background prefetch), falsy → 0 (synchronous buffer, paper-faithful eval). NOT for policies whose `select_action` consumes the observation every call (Diffusion Policy's obs history) — those keep the plain per-tick path. Replaced `queued_action_count` + the per-adapter fast-pop guards (removed — the executor buffer never touches the policy on pops, regardless of its private queue layout). (L556)
- `attach_chunk_executor(adapter, spec_extra, **kwargs) -> adapter` — Sets `adapter._chunk_executor = build_chunk_executor(...)` and returns the adapter, keeping every policy factory's tail a single statement. (L633)
- `to_numpy_action(action_tensor) -> NDArray[np.float32]` — `(1, A)` torch tensor → 1-D float32 NumPy. (L543)

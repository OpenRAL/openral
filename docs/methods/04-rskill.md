...
- `class ChunkedExecutor` — Overlaps GPU chunk inference with robot execution via a background daemon thread. Owns the per-step action buffer itself and calls `policy.predict_action_chunk` (never `select_action`/`reset` — a prefetch that reset the shared policy queue used to trigger a racing double inference at every chunk boundary). `select_action(batch_or_fn)` accepts a plain batch dict or a zero-arg callable, materialised only when an inference launches (buffer pops skip preprocessing entirely). `prefetch_at` default 15 (≈500 ms cover at 30 Hz). Policy-agnostic across lerobot ≥0.6 `PreTrainedPolicy` subclasses. (L61)
  - `__init__(policy, *, prefetch_at=15)` — Stash refs, no threads. (L86)
...
- `run_inference(policy, batch, *, chunk_index=None, kind="single", chunk_size=None, engine=None, method="select_action") -> Tensor` — Single seam wrapping a policy inference call in `inference_span` + `torch.no_grad()`; the only place `inference.kind` / `chunk_index` / `chunk_size` / `inference.engine` / `inference.device` attributes are emitted across both eval and skill paths. `engine` defaults to `"torch"`; `device` is auto-lifted from `policy.device` when present. `method` selects the policy entry point — `"select_action"` (queue-popping single step, default) or `"predict_action_chunk"` (pure chunk producer, used by `ChunkedExecutor`'s prefetch). (L487)
- `queued_action_count(policy) -> int` — Depth of a lerobot policy's internal `_queues["action"]` deque (0 for policies without the convention). The single place that touches the private queue layout; adapters use it to skip observation preprocessing on pop ticks (`_SmolVLAAdapter.step` fast path). (L543)

...
- `class ChunkedExecutor` — Owns the per-step action buffer for every chunked VLA family and optionally overlaps chunk N+1 inference with execution via a background daemon thread. Default producer is `policy.predict_action_chunk`; a custom `chunk_fn(payload) -> chunk` supports adapters with autocast, decode, or non-lerobot APIs. Tensor chunks consume the configured prefix; sequence producers must return exactly `chunk_size`, keeping scheduling and telemetry aligned. `select_action(batch_or_fn)` materialises lazy payloads only when an inference launches. `prefetch_at=0` is synchronous; `stop()` waits for a running inference before teardown. (L22)
  - `__init__(policy=None, *, chunk_fn=None, chunk_size=None, prefetch_at=15)` — policy OR chunk_fn+chunk_size; ValueError otherwise. (L25)
  - `start() -> None` — Mark as running (call after policy is on-device). (L87)
  - `stop() -> None` — Signal background thread, join. (L91)
  - `reset() -> None` — Clear buffer/bg state; `policy.reset()` when a policy was given. (L98)
  - `select_action(batch_or_fn) -> Any` — Next action; cold-start foreground inference / buffer pop / wait-on-prefetch. (L112)

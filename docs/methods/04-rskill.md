...
- `class ChunkedExecutor` — Overlaps GPU chunk inference with robot execution via a background daemon thread. The executor owns the foreground action deque and calls `predict_action_chunk` directly; it never resets or consumes lerobot's mutable `select_action` queue from two threads. The former shared-queue design reset the live queue during prefetch, reordered commands, and blocked 0.42-0.77 s at every 50-step boundary. (L61)
  - `__init__(policy, *, prefetch_at=20)` — Stash refs, no threads. Twenty remaining 30 Hz steps provide ~667 ms for the measured 313-600 ms chunk inference. (L86)
  - `start() -> None` — Mark as running (call after policy is on-device). (L120)
  - `stop() -> None` — Signal background thread, join. (L124)
  - `reset() -> None` — Reset state between episodes. (L132)
  - `select_action(batch) -> Any` — Next action, pre-fetching following chunk if needed. Foreground inference for step 1; pop-from-queue for step 2..N; wait-on-prefetch when queue drains. (L144)
  - private: `_launch_prefetch(batch)` (L219)
...
- `run_inference(policy, batch, *, chunk_index=None, kind="single", chunk_size=None, engine=None, method_name="select_action", synchronize=False) -> Tensor` — Single seam wrapping a named policy inference method in `inference_span` + `torch.no_grad()`; the only place `inference.kind` / `chunk_index` / `chunk_size` / `inference.engine` / `inference.device` attributes are emitted across both eval and skill paths. `ChunkedExecutor` passes `method_name="predict_action_chunk", synchronize=True`: CUDA launches are asynchronous, so without the explicit synchronization the background thread signalled ready before kernels completed and the first foreground action still paid the full 0.42-0.82 s boundary stall. `engine` defaults to `"torch"`; `device` is auto-lifted from `policy.device` when present. (L487)
- `to_numpy_action(action_tensor) -> NDArray[np.float32]` — `(1, A)` torch tensor → 1-D float32 NumPy. (L555)

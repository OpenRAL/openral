...
- `class ZmqSidecarMixin` (L17) — declares `_host`/`_port`/`_request_timeout_ms`/`_zmq`/`_ctx`/`_sock` as attributes a host class must set. `_connect()` — open a fresh `zmq.REQ` socket (`LINGER=0`, `RCVTIMEO`/`SNDTIMEO` set) to `tcp://{host}:{port}`. `_rpc(req, *, recv_timeout_ms=None) -> dict` — msgpack round-trip; recreates the socket on `zmq.error.Again` (REQ/REP can't recover from a missed reply) and re-raises. `_try_ping(*, recv_timeout_ms=1000) -> bool` — best-effort liveness check via `{"op": "ping"}`.
...
- `build_topreward_monitor(manifest, *, device="cuda") -> TOPRewardMonitor` — Build a `TOPRewardMonitor` from a `reward.backend == "topreward"` manifest (weights source, success threshold, target fps). Raises `ROSConfigError` on a non-reward manifest. (`backends/reward/topreward_reward.py` L212)
...
- `_percentile(samples: list[float], q: float) -> float` — Linear-interpolation percentile (`0.0` for empty list). Used by `_build_run_result`. (L46)
- `class InferenceRunnerBase(ABC)` — Owns the rate-limited loop, `rskill.tick` OTel parent span, `RunResult` aggregation, deadline-overrun policy. (L64)
  - `__init__(*, rate_hz=30.0, deadline_overrun_policy=WARN, runner_name="inference_runner", latency_budget_ms=None, save_dir=None)` — Reject `rate_hz <= 0`. (L100)
  - `activate() -> None` — Reset tick counter; mark active. (L127)
  - `deactivate() -> None` — Stop ticking; idempotent. (L132)
  - `_tick_impl(tick_idx: int) -> TickResult` [@abstractmethod] — Subclass hook; the base wraps it in a `rskill.tick` span. (L139)
  - `episode_start(task_string: str) -> int` — Optional explicit episode boundary; default raises `NotImplementedError`. `DeployRunner` overrides to drive the recorder; `SimRunner` overrides as a no-op (sim derives episode boundaries from `env.step` flags). (L176)
  - `episode_end(*, success: bool) -> None` — Optional explicit episode boundary; default raises `NotImplementedError`. See `episode_start`. (L201)
  - `_should_terminate() -> bool` — Subclass early-exit hook (default False) consulted after each tick inside `run()`. `SimRunner` overrides to stop once `n_episodes` complete. (L153)
  ...
  - `run(max_ticks: int | None = None) -> RunResult` — Rate-limited loop using `sleep_until`. Applies `DeadlineOverrunPolicy` (`warn` / `drop` / `raise`). Records `latency_budget_ms` violations and increments `openral.tick.budget_violations` per violation. Honors `_should_terminate()` after each tick. (L307)
  - `_current_trace_id() -> str | None` [@staticmethod] — Active OTel trace id (hex) or None. (L368)
  - `_on_deadline_overrun(result: TickResult) -> None` — Apply policy: structlog warn / drop / raise `ROSDeadlineMissed`. Always increments `openral.tick.deadline_misses` and emits `openral.event.deadline_missed` on the current parent span; on `RAISE`, also calls `record_exception` + `set_status(ERROR)` on the parent span before re-raising. The `WARN` / `DROP` **log lines** are rate-limited via `_deadline_log_due`; the metric and the span event are not. (L403)
  ...
  - `_build_run_result(results, *, budget_violations, trace_id) -> RunResult` — Aggregate per-tick records into `RunResult` (mean / p99). (L464)

...
- `parse_srdf_disabled_pairs(srdf_path) -> set[frozenset[str]]` (L80) — Parse a MoveIt SRDF's `<disable_collisions>` rows into unordered link pairs (the ACM).
- `fit_capsule_to_vertices(vertices, *, headroom_m=CAPSULE_HEADROOM_M) -> tuple[CapsuleShape, tuple[float×6]]` (L244) — Minimum-volume bounding capsule containing every vertex: axis searched over the principal axes + a 256-direction hemisphere + local refinement, the line through the minimal enclosing circle of the projection, the segment the shortest that still holds every vertex; the radius then grows by the declared `headroom_m`. Containment holds for every candidate axis by construction. Returns the shape + link-frame origin.
- `LinkFit` (L471) — Frozen dataclass: one link's fitted `primitives` (`(shape, origin)` list), the winning `kind` (`capsule` / `box` / `capsule×k`), `volume_ratio` (union ÷ hull volume), `max_protrusion_m` / `mean_protrusion_m` (largest / mean support excess over the hull), `candidates` (`{kind: (volume_ratio, max_protrusion_m, mean_protrusion_m)}`) and `fitted` (every candidate's primitives, for re-picking in a review).
- `fit_link_primitives(mesh, *, headroom_m=CAPSULE_HEADROOM_M, max_capsules=3, force_box=False) -> LinkFit` (L510) — The one fitting step both lowering paths call on a link's `trimesh.Trimesh` (collision + visual, link frame): fits one capsule (`fit_capsule_to_vertices`), one box (`fit_obb_to_vertices`) and capsule chains of 2..`max_capsules` (the mesh cut into equal slabs along the capsule axis, each slab fitted), every candidate holding every vertex with the headroom; picks the least **mean protrusion** (support excess over the convex hull, averaged over directions), ties within `_PROTRUSION_TIE_FRACTION` going to the cheaper kernel kind (capsule < fewer capsules < box); excess volume and max protrusion are reported per candidate. `force_box` for a link that will carry `tight_geometry`.
- `fit_link_with_tight_geometry(link_name, mesh, *, headroom_m=CAPSULE_HEADROOM_M) -> LinkCollisionGeometry` (L601) — One link as a box (`fit_obb_to_vertices`, origin rounded and half-extents rounded up to the manifest's 4 dp) plus the `tight_geometry` derived in that rendered frame (`tight_geometry.tight_geometry_for_box`), so `mesh ⊆ hull ⊆ DOP ⊆ box` holds for the box the kernel loads. What `--tight-link` and a manifest-refined link lower to.
- module constant `_RENDER_DP: int = 4` (L598) — the precision `fit_link_with_tight_geometry` rounds the box to before deriving its refinement (mirrors `openral_cli.collision._BLOCK_FLOAT_DP`).
- `_rendered(shape, origin) -> tuple[CollisionShape, origin]` (L647) — A primitive rounded to the manifest's 4 dp; `_fit_link` returns every fitted primitive this way so the ACM certificate and the regression test reason about the numbers the kernel loads, not an unrounded twin.
- `_fit_link(link_name, mesh, tight_links) -> list[LinkCollisionGeometry]` (L668) — One link's entries: `fit_link_with_tight_geometry` for a link in `tight_links`, else `fit_link_primitives` at manifest precision; the step both readers share.
- `_tight_links_of(robot, extra) -> frozenset[str]` (L685) — Links to lower as box + hull: every link the manifest already refines plus `extra` — sticky, so `collision check` reproduces a refined manifest and dropping a refinement is a manifest edit.
- `_twin_link_meshes(robot, urdf_model, manifest_dir) -> dict[str, Any]` (L1973) — The robot's MJCF twin mesh geoms per manifest link, in the link frame, when every mapped twin body frame coincides with its URDF link frame at zero (`_TWIN_FRAME_TOL = 1e-6`); `lower_robot` folds them into the fit so the primitives hold the simulated robot's CAD too (the menagerie H1's forearm is 46 mm longer than h1_description's). Empty (and logged) when the frames differ, `mujoco` is missing or the robot has no twin.
- module constant `_PROTRUSION_TIE_FRACTION: float = 0.03` (L436), `_PROTRUSION_DIRS: int = 1024` (L434), `_UNION_VOLUME_SAMPLES: int = 60_000` (L413) — `fit_link_primitives`'s tie window, support-excess direction count and fixed-seed Monte-Carlo sample count for a chain's union volume.
- `CAPSULE_HEADROOM_M = 0.001` (L130) — declared clearance every URDF-lowered capsule keeps around its link's geometry; absorbs the manifest's 4-dp rendering (`tests/unit/test_collision_geometry_enclosure_urdf.py`).
- `lower_link_geometry(urdf_path, *, tight_links=()) -> list[LinkCollisionGeometry]` (L759) — The primitive set of every URDF link with a `<collision>`, fitted by `fit_link_primitives` to the link's `<collision>` **and** `<visual>` geometry together (box→8 corners, cylinder/sphere→circumscribing polytope, mesh→trimesh); a lone sphere `<collision>` with no visual stays an exact sphere. A link may yield several entries (a capsule chain); a link in `tight_links` lowers to `fit_link_with_tight_geometry` instead. Raises `ROSConfigError` when a link's geometry resolves only partially; skips a link none of whose geometry resolves.
- module constant `_CERTIFY_MAX_DOF: int = 5` (L953)
- module constant `_CERTIFY_MAX_CELLS: int = 400_000` (L957)
- module constant `_CERTIFY_MAX_LEVELS: int = 96` (L960)
- module constant `_COARSE_NODES: int = 7` (L962) — the four `_certified_always_colliding` cost knobs (see the module intro); hitting any withholds the ACM entry, never soundness.
- module constant `_MJCF_RNG_SEED: int = 20260610` (L970)
- module constant `_MJCF_N_SAMPLES: int = 2000` (L971) — defaults for `lower_robot_from_mjcf`'s seeded always-colliding sweep.
- `acm_for_geometry(urdf_path, geoms, *, srdf_path=None, margin_m=0.0) -> set[frozenset[str]]` (L1302) — The ACM for the exact primitives the kernel will load (a flat list, or a mapping by link to one primitive or its list), decided with the kernel's own predicates at the robot's own margin, each link pair's gap folded over its primitives. `ACM = adjacent ∪ always-colliding ∪ [SRDF-disabled if given]` — geometry can prove a pair always collides but never that it never does, so without an SRDF a sometimes-colliding pair stays checked. Deterministic, no RNG.
- `sample_acm_from_urdf(urdf_path, *, margin_m=0.0) -> set[frozenset[str]]` (L1368) — No-SRDF fallback: `adjacent ∪ always-colliding` from the URDF's own geometry. Conservative against a precise-mesh SRDF (its disabled set is a subset, never false-permissive). The name is now a misnomer (nothing is sampled) but is kept as public API/provenance value.
- `class LoweredCollisionModel` (L1397) — Frozen dataclass result: `collision_geometry` (several entries per link allowed), `allowed_collision_pairs` (sorted tuples), `acm_source` (`"srdf"`|`"sampling"`|`"mjcf"`), `srdf_path`, `joint_fk`.
- `lower_joint_fk(robot, urdf_ref) -> dict[str, tuple[xyz, rpy, axis]]` (L1473) — Per-manifest-joint forward kinematics read from the URDF, matched by `child_link`; needed to place the link capsules. Unmatched (synthetic) joints are omitted.
- `lower_robot_from_mjcf(robot, *, n_samples=2000, seed=20260610, margin_m=0.0, manifest_dir=None, acm_only=False, tight_links=None) -> LoweredCollisionModel` (L1795) — MJCF backend for robots with no URDF collision geometry (e.g. bimanual `openarm`): fits the geometry with `fit_collision_geometry_from_mjcf` (kept from the manifest under `acm_only`, as `lower_robot` keeps it), lowers joint FK from mujoco, and derives a conservative ACM from a seeded mujoco-FK overlap sweep over that geometry, folding each link pair's gap over its primitives as the kernel does, and never auto-disabling a pair whose two links both carry a hull (the kernel re-asks those of the hulls) — the one remaining sampled criterion, since the URDF joint-tree proof doesn't apply here. A manifest `assets.srdf` is unioned into the sweep's disabled set for hand-reviewed exemptions the sweep can't prove; hand-editing the generated ACM block is forbidden. `acm_source="mjcf"`.
- module constant `_MOVABLE_JOINT_TYPES: tuple[str, ...]` (L1069) — `("revolute", "continuous", "prismatic")`, joint types `lower_joint_fk`/`acm_for_geometry` treat as movable.
- `lower_robot(robot, *, srdf_path=None, acm_only=False, geometry_only=False, manifest_dir=None, tight_links=None) -> LoweredCollisionModel` (L2066) — Top-level entry; threads `robot.safety.self_collision_margin_m` into `acm_for_geometry` so the ACM matches the kernel's live threshold. ACM source precedence: explicit `srdf_path` → manifest `assets.srdf` → URDF-only fallback. Raises `ROSConfigError` on zero fitted links, never an empty geometry/ACM the kernel would silently not check.
- `select_lowering(robot, *, manifest_dir=None) -> LoweringSource` (L2235) — Provenance-correct routing: `"srdf"` (SRDF + usable URDF geometry), `"sampling"` (URDF geometry, no SRDF), `"mjcf"` (no usable URDF geometry but an MJCF exists, e.g. `openarm`'s unresolved package mesh refs). Raises `ROSConfigError` when no lowerable asset exists.
- `lower_robot_auto(robot, *, acm_only=False, geometry_only=False, manifest_dir=None, tight_links=None) -> LoweredCollisionModel` (L2280) — Single dispatch over `select_lowering` → `lower_robot` or `lower_robot_from_mjcf` (`acm_only` keeps the manifest geometry on both paths; `tight_links` adds box + hull links on top of those the manifest already refines, `_tight_links_of`); the one entry point the CLI and the regression test both call, so routing can never diverge.
- `fit_obb_to_vertices(vertices, *, headroom_m=CAPSULE_HEADROOM_M) -> tuple[BoxShape, tuple[float×6]]` (L343) — Minimum-volume oriented bounding box (`trimesh.bounds.oriented_bounds` over the hull), half-extents re-measured from the cloud in that frame and grown by `headroom_m`; every vertex inside by construction.
- module constant `_MJCF_STROKE_SAMPLES: int = 9` (L977) — leader positions `fit_collision_geometry_from_mjcf` sweeps a coupled follower over.
- `mjcf_coupled_joints(model) -> dict[int, list[tuple[int, float, float]]]` (L1623) — MuJoCo joint equalities (`q[obj1] = c0 + c1·q[obj2]`) as a two-way map, joint id → `[(other joint, c0, c1)]`, so a follower is set correctly whichever side the manifest drives (OpenArm drives `obj1`). Raises `ROSConfigError` on a non-linear or non-invertible coupling.
- `fit_collision_geometry_from_mjcf(robot, *, manifest_dir=None, stroke_samples=9, tight_links=()) -> list[LinkCollisionGeometry]` (L1703) — `fit_link_primitives` per manifest link over every geom of its MJCF body — collision **and** visual, meshes and primitives — in the body frame. An equality-coupled follower body (OpenArm's second finger) is folded into its leader's child link, swept over `stroke_samples` leader positions, so the primitives hold the gripper at every opening. Pinned by `tests/unit/test_collision_geometry_enclosure.py`.
...
### `packages/openral_safety/openral_safety/tight_geometry.py`

_Derive a box's `tight_geometry` (26-DOP + exact hull) from the geometry it bounds; containment is definitional (`mesh ⊆ hull ⊆ DOP ⊆ box`). Moved here from `tools/generate_tight_geometry.py` so the collision lowering can refine links; the tool re-exports the moved names._

- `derive_tight_geometry(points, half_extents) -> dict[str, Any]` (L269) — The DOP slabs and the exact hull, refined toward the hull under the `MAX_TIGHT_HULL_VERTICES` budget (`refine_dop_to_budget`) when it is over; a mapping for `TightCollisionGeometry` plus reviewer diagnostics.
- `refine_dop_to_budget(points, dop_lo, dop_hi, budget) -> Points` (L187) — A ≤`budget`-vertex envelope built only from tangent planes of `conv(points)`, starting from the DOP; refuses to emit one that cuts the points.
- `hull_overhang_m(hull_points, mesh_points, mesh_faces, *, samples_per_edge=24) -> float` (L106) — Sampled distance from the hull's facets to the real mesh surface (exact per sample, brute force, batched).
- `hull_overhang_upper_bound_m(hull_points, mesh, *, samples_per_edge=24) -> float` (L156) — The same measurement from above via a KD-tree over a fixed-seed surface sample, for CAD meshes too large for the brute-force query.
- `tight_geometry_for_box(mesh, half_extents) -> TightCollisionGeometry` (L309) — DOP + hull + padded overhang for a box-frame mesh, written at 1 nm (slabs rounded outward) so a re-lowering reproduces it byte for byte.
- `round_up_m(value, precision_m=1e-6) -> float` (L68) — Round a sampled distance up, never down.
- module constant `HULL_OVERHANG_SAFETY_MARGIN: float = 1.2` (L65) — pad on a sampled overhang; `_OVERHANG_BATCH`, `_OVERHANG_MAX_SAMPLES`, `_SURFACE_SAMPLES`, `_SURFACE_SEED`, `_WRITE_DP` — the query's memory bound, sample caps, seed and write precision.
...
- `sphere_model_geometry(geoms) -> dict[str, list[LinkCollisionGeometry]]` (L156) — Each link's primitives as the capsules cuRobo's spheres actually cover (box → containing capsule; capsules/spheres unchanged) — a strictly larger solid than the kernel's box, so it needs its own ACM proof.
- `actuated_joint_names(robot) -> list[str]` (L206) — Single-DOF movable joint names, in manifest order — the cuRobo `cspace.joint_names`.
- module constant `_SPHERE_DP: int = 6` (L215) — decimal places sphere centers/radii are rounded to when rendered into the YAML fragment.
- `render_cumotion_config(robot, model, *, urdf_path=None) -> str` (L218) — Render a cuRobo `robot_cfg` YAML fragment with a generated-provenance header. Spheres come from `robot.collision_geometry` (falling back to `model.collision_geometry` only when the manifest carries none). The ACM is not copied verbatim: given `urdf_path` it is widened with pairs proven always-colliding for the sphere model, so the planner stays looser than the kernel, never tighter. Without `urdf_path` the manifest ACM is used as-is and the header says so.
...
- `emit_sensor_frame_span(frame, *, sensor_name, age_ms, flip_180=False, tracer_name=…) -> None` — The shared producer of a dashboard `sensors.read_latest` span for one camera frame: optional flip of a display-only copy (the policy's actual frame is never mutated), then span + attrs + thumbnail. Both frame emitters route through this one function so they can't drift apart. (L245) `encode_frame_thumbnail` — but the span is opened first and the function returns as soon as `span.is_recording()` is false (no OTLP endpoint / dashboard off): the flip copy and the JPEG only feed the span, and at 30 Hz per camera they held ~25 % of the deploy runtime's GIL on an AGX Orin for nothing. The flip is passed to `encode_frame_thumbnail`; the span no longer copies a full-resolution flipped frame.
- `encode_rgb_thumbnail(rgb) -> bytes | None` — Encode an HWC uint8 RGB ndarray to a small JPEG for OTLP; returns `None` if Pillow is unavailable. (L300)
- `encode_frame_thumbnail(frame, *, flip_180=False) -> bytes | None` — Encode an `openral_core.SensorFrame` (RGB8/BGR8/MONO8/JPEG/PNG) as a small JPEG thumbnail; returns `None` for non-renderable encodings. (L333) Raw frames are subsampled to the 320x240 box before any copy, and the optional 180° flip and BGR swap run on that small view: 0.6 ms per 1920x1200 frame on a Jetson AGX Thor instead of 8.2 ms, which at three cameras x 30 Hz had left a competing Python thread 34 % of its speed. JPEG input decodes in reduced-scale draft mode. Display-only (the dashboard tile); Foxglove reads the full-resolution ROS image topics.
...
- `install_structlog_bridge(logger_provider)` — Wire the structlog processor chain to forward records to the OTel `LoggerProvider`, with both the bridge logger and the `openral` root logger set to `resolve_log_level()`. (L129)

...
- `acm_for_geometry(urdf_path, geoms, *, srdf_path=None, margin_m=0.0) -> set[frozenset[str]]` (L1299) — The ACM for the exact primitives the kernel will load (a flat list, or a mapping by link to one primitive or its list), decided with the kernel's own predicates at the robot's own margin, each link pair's gap folded over its primitives. `ACM = adjacent ∪ always-colliding ∪ [SRDF-disabled if given]` — geometry can prove a pair always collides but never that it never does, so without an SRDF a sometimes-colliding pair stays checked. Deterministic, no RNG.
- `sample_acm_from_urdf(urdf_path, *, margin_m=0.0) -> set[frozenset[str]]` (L1365) — No-SRDF fallback: `adjacent ∪ always-colliding` from the URDF's own geometry. Conservative against a precise-mesh SRDF (its disabled set is a subset, never false-permissive). The name is now a misnomer (nothing is sampled) but is kept as public API/provenance value.
- `class LoweredCollisionModel` (L1394) — Frozen dataclass result: `collision_geometry` (several entries per link allowed), `allowed_collision_pairs` (sorted tuples), `acm_source` (`"srdf"`|`"sampling"`|`"mjcf"`), `srdf_path`, `joint_fk`.
- `lower_joint_fk(robot, urdf_ref) -> dict[str, tuple[xyz, rpy, axis]]` (L1470) — Per-manifest-joint forward kinematics read from the URDF, matched by `child_link`; needed to place the link capsules. Unmatched (synthetic) joints are omitted.
- `lower_robot_from_mjcf(robot, *, n_samples=2000, seed=20260610, margin_m=0.0, manifest_dir=None, acm_only=False, tight_links=None) -> LoweredCollisionModel` (L1792) — MJCF backend for robots with no URDF collision geometry (e.g. bimanual `openarm`): fits the geometry with `fit_collision_geometry_from_mjcf` (kept from the manifest under `acm_only`, as `lower_robot` keeps it), lowers joint FK from mujoco, and derives a conservative ACM from a seeded mujoco-FK overlap sweep over that geometry, folding each link pair's gap over its primitives as the kernel does, and never auto-disabling a pair whose two links both carry a hull (the kernel re-asks those of the hulls) — the one remaining sampled criterion, since the URDF joint-tree proof doesn't apply here. A manifest `assets.srdf` is unioned into the sweep's disabled set for hand-reviewed exemptions the sweep can't prove; hand-editing the generated ACM block is forbidden. `acm_source="mjcf"`.
...
- `lower_robot(robot, *, srdf_path=None, acm_only=False, geometry_only=False, manifest_dir=None, tight_links=None) -> LoweredCollisionModel` (L2063) — Top-level entry; threads `robot.safety.self_collision_margin_m` into `acm_for_geometry` so the ACM matches the kernel's live threshold. ACM source precedence: explicit `srdf_path` → manifest `assets.srdf` → URDF-only fallback. Raises `ROSConfigError` on zero fitted links, never an empty geometry/ACM the kernel would silently not check.
- `select_lowering(robot, *, manifest_dir=None) -> LoweringSource` (L2232) — Provenance-correct routing: `"srdf"` (SRDF + usable URDF geometry), `"sampling"` (URDF geometry, no SRDF), `"mjcf"` (no usable URDF geometry but an MJCF exists, e.g. `openarm`'s unresolved package mesh refs). Raises `ROSConfigError` when no lowerable asset exists.
- `lower_robot_auto(robot, *, acm_only=False, geometry_only=False, manifest_dir=None, tight_links=None) -> LoweredCollisionModel` (L2277) — Single dispatch over `select_lowering` → `lower_robot` or `lower_robot_from_mjcf` (`acm_only` keeps the manifest geometry on both paths; `tight_links` adds box + hull links on top of those the manifest already refines, `_tight_links_of`); the one entry point the CLI and the regression test both call, so routing can never diverge.

...
- `class AnthropicToolUseClient` (L882) — Anthropic SDK-backed client. `__init__(*, model_id, api_key, max_tokens=1024, timeout_s=10.0)`. Lazy-imports `anthropic`.
- `class OpenAICompatibleToolUseClient` (L961) — OpenAI SDK-backed client pointed at any OpenAI-protocol endpoint (cloud OpenAI, local vLLM, Ollama-OpenAI). `__init__(*, model_id, api_key=None, base_url=None, timeout_s=10.0)`. Lazy-imports `openai`.
...
- `_drop_property(schema: dict, name: str) -> dict` — Return a copy of a JSON Schema dict with `name` stripped from both `properties` and `required`. Used to drop `rskill_id` from per-skill `ExecuteRskillTool` schemas. (L791)

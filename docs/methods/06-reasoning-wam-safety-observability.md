...
- `parse_srdf_disabled_pairs(srdf_path) -> set[frozenset[str]]` (L75) — Parse a MoveIt SRDF's `<disable_collisions>` rows into unordered link pairs (the ACM).
- `fit_capsule_to_vertices(vertices) -> tuple[CapsuleShape, tuple[float×6]]` (L118) — PCA bounding capsule containing every vertex, a conservative over-approximation; returns the shape + link-frame origin.
- `fit_trimmed_capsule_to_vertices(vertices) -> tuple[CapsuleShape, tuple[float×6]]` (L182) — The shortest enclosing capsule on the PCA major axis: the segment is trimmed so the end caps, not the cylinder, cover the extreme vertices. Tighter than `fit_capsule_to_vertices` on stubby links; still encloses every vertex.
- `fit_obb_to_vertices(vertices) -> tuple[BoxShape, tuple[float×6]]` (L215) — PCA-oriented bounding box over the vertices.
- `fit_tightest_primitive(vertices) -> tuple[CapsuleShape | BoxShape, tuple[float×6]]` (L229) — The smaller-volume of the trimmed capsule and the PCA box; both enclose every vertex, so either is conservative.
- `lower_link_geometry(urdf_path) -> list[LinkCollisionGeometry]` (L289) — One conservative capsule/sphere per URDF link with a `<collision>` (box→8 corners, cylinder→cap rims, sphere→exact, mesh→PCA-fit trimesh vertices).
- module constant `_CERTIFY_MAX_DOF: int = 5` (L412)
- module constant `_CERTIFY_MAX_CELLS: int = 400_000` (L416)
- module constant `_CERTIFY_MAX_LEVELS: int = 96` (L419)
- module constant `_COARSE_NODES: int = 7` (L421) — the four `_certified_always_colliding` cost knobs (see the module intro); hitting any withholds the ACM entry, never soundness.
- module constant `_MJCF_RNG_SEED: int = 20260610` (L429)
- module constant `_MJCF_N_SAMPLES: int = 2000` (L430) — defaults for `lower_robot_from_mjcf`'s seeded always-colliding sweep.
- module constant `_MJCF_STROKE_SAMPLES: int = 9` (L436) — leader positions `fit_collision_geometry_from_mjcf` sweeps a coupled follower over.
- `acm_for_geometry(urdf_path, geoms, *, srdf_path=None, margin_m=0.0) -> set[frozenset[str]]` (L707) — The ACM for the exact per-link primitives the kernel will load, decided with the kernel's own predicates at the robot's own margin. `ACM = adjacent ∪ always-colliding ∪ [SRDF-disabled if given]` — geometry can prove a pair always collides but never that it never does, so without an SRDF a sometimes-colliding pair stays checked. Deterministic, no RNG.
- `sample_acm_from_urdf(urdf_path, *, margin_m=0.0) -> set[frozenset[str]]` (L770) — No-SRDF fallback: `adjacent ∪ always-colliding` from the URDF's own geometry. Conservative against a precise-mesh SRDF (its disabled set is a subset, never false-permissive). The name is now a misnomer (nothing is sampled) but is kept as public API/provenance value.
- `class LoweredCollisionModel` (L798) — Frozen dataclass result: `collision_geometry`, `allowed_collision_pairs` (sorted tuples), `acm_source` (`"srdf"`|`"sampling"`|`"mjcf"`), `srdf_path`, `joint_fk`, `geometry_fitted` (an MJCF lowering that fitted its geometry, so the CLI writes it).
- `lower_joint_fk(robot, urdf_ref) -> dict[str, tuple[xyz, rpy, axis]]` (L878) — Per-manifest-joint forward kinematics read from the URDF, matched by `child_link`; needed to place the link capsules. Unmatched (synthetic) joints are omitted.
- `mjcf_coupled_joints(model) -> dict[int, list[tuple[int, float, float]]]` (L978) — MuJoCo joint equalities (`q[obj1] = c0 + c1·q[obj2]`) as a two-way map, joint id → `[(other joint, c0, c1)]`, so a follower is set correctly whichever side the manifest drives (OpenArm drives `obj1`). Raises `ROSConfigError` on a non-linear or non-invertible coupling.
- `fit_collision_geometry_from_mjcf(robot, *, manifest_dir=None, stroke_samples=9) -> list[LinkCollisionGeometry]` (L1053) — One `fit_tightest_primitive` per manifest link over its MJCF collision-mesh vertices, in the body frame. An equality-coupled follower body (OpenArm's second finger) is folded into its leader's child link, swept over `stroke_samples` leader positions, so the primitive holds the gripper at every opening. Pinned by `tests/unit/test_collision_geometry_enclosure.py`.
- `lower_robot_from_mjcf(robot, *, n_samples=2000, seed=20260610, margin_m=0.0, manifest_dir=None, fit_geometry=False) -> LoweredCollisionModel` (L1144) — MJCF backend for robots with no URDF collision geometry (e.g. bimanual `openarm`): keeps the manifest's geometry, or with `fit_geometry=True` replaces it by `fit_collision_geometry_from_mjcf`; lowers joint FK from mujoco, and derives a conservative ACM from a seeded mujoco-FK overlap sweep over that geometry — the one remaining sampled criterion, since the URDF joint-tree proof doesn't apply here. A manifest `assets.srdf` is unioned into the sweep's disabled set for hand-reviewed exemptions the sweep can't prove; hand-editing the generated ACM block is forbidden. `acm_source="mjcf"`, `geometry_fitted=fit_geometry`.
- module constant `_MOVABLE_JOINT_TYPES: tuple[str, ...]` (L483) — `("revolute", "continuous", "prismatic")`, joint types `lower_joint_fk`/`acm_for_geometry` treat as movable.
- `lower_robot(robot, *, srdf_path=None, acm_only=False, geometry_only=False, manifest_dir=None) -> LoweredCollisionModel` (L1303) — Top-level entry; threads `robot.safety.self_collision_margin_m` into `acm_for_geometry` so the ACM matches the kernel's live threshold. ACM source precedence: explicit `srdf_path` → manifest `assets.srdf` → URDF-only fallback. Raises `ROSConfigError` on zero fitted links, never an empty geometry/ACM the kernel would silently not check.
- `select_lowering(robot, *, manifest_dir=None) -> LoweringSource` (L1457) — Provenance-correct routing: `"srdf"` (SRDF + usable URDF geometry), `"sampling"` (URDF geometry, no SRDF), `"mjcf"` (no usable URDF geometry but an MJCF exists, e.g. `openarm`'s unresolved package mesh refs). Raises `ROSConfigError` when no lowerable asset exists.
- `lower_robot_auto(robot, *, acm_only=False, geometry_only=False, manifest_dir=None, fit_mjcf_geometry=False) -> LoweredCollisionModel` (L1502) — Single dispatch over `select_lowering` → `lower_robot` or `lower_robot_from_mjcf` (forwarding `fit_mjcf_geometry`, ignored on the URDF path); the one entry point the CLI and the regression test both call, so routing can never diverge.

...
- module constant `OPENROUTER_BASE_URL: str` — `https://openrouter.ai/api/v1`; pre-filled when `PROVIDER=openrouter` so users don't have to memorise it. (L446)
- module constant `OLLAMA_BASE_URL: str` — `http://localhost:11434/v1`; pre-filled when `PROVIDER=ollama` (no auth). (L455)
- module constants `GEMINI_BASE_URL` / `XAI_BASE_URL` / `DEEPSEEK_BASE_URL: str` — vendor OpenAI-compatible endpoints pre-filled when `PROVIDER=gemini` / `xai` / `deepseek` (issue #74; auth-required thin presets over `OpenAICompatibleToolUseClient`). (L465)
- module constant `_OPENAI_COMPATIBLE_PRESETS: dict[str, str]` — auth-required named presets (`openrouter` / `gemini` / `xai` / `deepseek`) → default base URL; the factory iterates this so the three new vendors reuse one branch. (L473)
- module constant `SYSTEM_PROMPT_ENV_VAR: str = "OPENRAL_REASONER_SYSTEM_PROMPT"` (L342) — env var that overrides the base operating brief; honoured by `resolve_reasoner_system_prompt`.
- `render_robot_context_prompt(capabilities: RobotCapabilities | None, *, base_prompt=DEFAULT_SYSTEM_PROMPT) -> str` (L231) — Option B: append a deterministic `## THIS ROBOT` body-awareness block (embodiment tags, locomotion + navigate/no-navigate guidance, manipulation/sensing hardware, payload, control modes) to the system prompt. `None` returns `base_prompt` unchanged.
- `resolve_reasoner_system_prompt(capabilities: RobotCapabilities | None, *, env=None) -> str` (L345) — Compose the reasoner system prompt: base brief (`OPENRAL_REASONER_SYSTEM_PROMPT` override if non-empty, else `DEFAULT_SYSTEM_PROMPT`) + the `## THIS ROBOT` block. `env` is injectable for tests. Called by `ReasonerNode.on_configure`.
- `class ToolUseClient(Protocol)` (L392) — Attribute `model_id`; method `select_tool(*, context_text, palette, system_prompt=DEFAULT_SYSTEM_PROMPT) -> ReasonerToolCall`. Raises `ROSReasonerInvalidPlan` on bad discriminator / palette mismatch, `ROSPlanningError` on transport failure.
- `build_tool_use_client_from_env() -> ToolUseClient` (L487) — Factory reading `OPENRAL_REASONER_LLM_{PROVIDER, MODEL, API_KEY, BASE_URL}`. PROVIDER ∈ {`anthropic`, `openai-compatible`, `ollama`, `openrouter`, `gemini`, `xai`, `deepseek`}; `openrouter` / `gemini` / `xai` / `deepseek` are auth-required shortcuts on top of `OpenAICompatibleToolUseClient` that pre-fill the vendor base URL (`_OPENAI_COMPATIBLE_PRESETS`), `ollama` pins the local loopback URL with no key. No cloud lock-in: open-core has no default.
- `class AnthropicToolUseClient` (L957) — Anthropic SDK-backed client. `__init__(*, model_id, api_key, max_tokens=1024, timeout_s=10.0)`. Lazy-imports `anthropic`.
- `class OpenAICompatibleToolUseClient` (L1036) — OpenAI SDK-backed client pointed at any OpenAI-protocol endpoint (cloud OpenAI, local vLLM, Ollama-OpenAI). `__init__(*, model_id, api_key=None, base_url=None, timeout_s=10.0)`. Lazy-imports `openai`.
...
- module constant `_PER_SKILL_TOOL_PREFIX: str = "execute_rskill__"` — prefix the decoder matches on to identify per-skill tool calls. (L596)
- module constant `_LLM_TOOL_NAME_MAX_LEN: int = 64` — Anthropic + OpenAI tool-name regex limit; long HF Hub ids are sha1-suffix-truncated to fit. (L599)
- `_skill_id_to_tool_name(rskill_id: str) -> str` — Slugify a `<owner>/<repo>` HF Hub id into a 64-char-max LLM tool name. Long ids get an 8-char sha1 suffix to stay unique post-truncation. (L602)
- `_format_skill_tool_description(entry: RSkillToolEntry) -> str` — Render the skill's id + description + actions + objects + scenes into the NL string the LLM scores. (L625)
- `_drop_property(schema: dict, name: str) -> dict` — Return a copy of a JSON Schema dict with `name` stripped from both `properties` and `required`. Used to drop `rskill_id` from per-skill `ExecuteRskillTool` schemas. (L866)
...
- `class DiscoveredRobot(BaseModel)` — One mDNS-discovered OpenRAL service; the `/api/robots` wire shape. Fields: `name: str`, `addresses: list[str]`, `port: int`, `properties: dict[str, str] = {}`, `last_seen: float`. (L37)

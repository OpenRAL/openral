...
- `resolve_weights_file(repo_id, *, filename="model.safetensors") -> str` — Local path of the weights for `repo_id`, whether that is the **directory** `resolve_rskill_to_hf_with_revision` returns for an `openral rskill install`-ed snapshot (file taken from the directory) or a bare Hub repo id (cached-first download). The one place that tells the two apart; `peek_safetensors_keys`, `_load_bf16_state_for_int8` and `_stream_bf16_state_to_device` all route through it — every direct `hf_hub_download` broke on the directory case.

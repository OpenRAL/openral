...
- `_pi05_phase(name, **fields) -> ContextManager[None]` — Adapter-local shortcut for `phase_timer(name, prefix="pi05", gpu_mb=True, log=_log)`. (L459)
...
- `_load_bf16_state_for_int8(policy, repo_id, *, torch) -> None` — Downloads `<repo>/model.safetensors` via `resolve_weights_file` (directory-aware) and applies it via `policy.load_state_dict(strict=False)`. The int8 fast meta-init path's substitute for lerobot's ~152 s `PI05Policy.from_pretrained`. (L411)
...
- `_resolve_pretrained_path(spec, repo_id) -> str` — Returns a local directory containing the lerobot processor sidecars. Local path → verbatim; otherwise routes through `_processors.resolve_processor_dir`. (L473)

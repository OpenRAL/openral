...
- `_rebuild_gemma_buffer(name, mod, buf, *, torch) -> bool` — π0.5's `BufferRebuilder` hook: RoPE `*.inv_freq` / `*.original_inv_freq` (theta from the module's `config.rope_parameters`, else its `rope_theta` / `base`, else 10000) and Gemma's `*.embed_scale` (`sqrt(hidden_size)`, from the module's surviving `scalar_embed_scale`, else `sqrt(embedding_dim)`). An `embed_scale` on a module with neither returns `False`, so the shared refusal names it instead of an `AttributeError`. (L306)
- `_init_pi05_buffers(policy, *, torch) -> None` — `rebuild_non_persistent_buffers(..., family="pi05", rebuild=_rebuild_gemma_buffer)`; shared by the nf4, int8 and bf16 fast meta-init paths. (L348)

...
- `_init_rope_and_position_buffers(policy, *, torch) -> None` — Rebuilds `*.position_ids` (int64 arange) and RoPE `*.inv_freq` / `*.original_inv_freq` after a meta init; the one body the nf4, int8 and bf16 fast paths share (was inlined twice).

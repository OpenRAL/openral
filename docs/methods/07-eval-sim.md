...
- `const _PACKING_TOKENS: frozenset[str] = frozenset({'nf4', 'int8'})` — The two tokens for which `QuantPlan.quantize` is `True`. (L678)
- `const _QUANT_TOKEN_ALIASES: dict[str, str]` — Dtype-spelling normalization table consumed by `canonical_quant_token` (`int4`/`4bit` → `nf4`, float aliases fold, …). (L685)
...
- `peek_safetensors_keys(repo_id, *, filename="model.safetensors") -> set[str] | None` — Reads only the safetensors header and returns its key set, for both prequantized (nf4) and bare (int8) checkpoints. Used by `targeted_reset_parameters` to skip the init walk for modules whose params the upcoming state load will overwrite anyway. (L598)
- `resolve_weights_file(repo_id, *, filename="model.safetensors") -> str` (L571) — Local path of the weights for `repo_id`, whether that is the **directory** `resolve_rskill_to_hf_with_revision` returns for an `openral rskill install`-ed snapshot (file taken from the directory) or a bare Hub repo id (cached-first download). The one place that tells the two apart; `peek_safetensors_keys`, `_load_bf16_state_for_int8` and `_stream_bf16_state_to_device` all route through it — every direct `hf_hub_download` broke on the directory case.
- `targeted_reset_parameters(policy, *, covered_keys) -> None` — Calls `module.reset_parameters()` only on modules whose params are NOT covered by the about-to-load state dict (`covered_keys=None` reproduces the unconditional reset). Model-agnostic, shared by π0.5, MolmoAct2, and future meta-init families. (L949)
- `tie_transformers_weights(policy) -> None` — Calls `module.tie_weights()` on each outermost transformers backbone (pre-order, skipping already-tied descendants); a raising `tie_weights` is non-fatal. Shared by π0.5 and MolmoAct2. (L1009)
- `normalise_manifest_dtype(manifest) -> str | None` — Pulls `manifest.quantization.dtype.value` as a string; returns `None` for manifests without a quantization block. Lifted out of `pi05.py` into the shared module so smolvla / xvla / future quantized adapters share one implementation. (L649)
- `QUANTIZATION_DTYPE_ENV: str` — `"OPENRAL_QUANTIZATION_DTYPE"` — the one all-family per-run dtype override, replacing the old per-family `OPENRAL_GR00T_QUANTIZATION`/`OPENRAL_RLDX_QUANTIZATION` env vars. (L673)
- `canonical_quant_token(raw) -> str | None` — Normalises a dtype spelling to the one token every consumer understands (`int4`/`4bit` → `nf4`, float aliases fold, unknown tokens pass through lowercased). `None` stays `None` so “unset” is distinguishable from an explicit `"none"` (= do not quantize). (L702)
- `QuantPlan(dtype, quantize, source, extra)` — `NamedTuple` returned by `resolve_quant_plan`. `quantize` is `True` only for a packing token (`nf4` / `int8`), which is the single definition of "does this load rewrite Linears"; `source` is `env` / `spec_quantization` / `spec_extra` / `manifest` / `default` / `unset` so the adapter can log what won; `extra` carries the manifest's `quantization.extra` knobs. (L726)
- `quantization_extra(manifest) -> Mapping[str, Any]` — Returns `manifest.quantization.extra` (the `quantize_scope` / `nf4_min_params` knobs), or `{}`. One home for the packing knobs, which used to sit in `policy_extras` next to unrelated adapter settings — which is why each family grew its own reader. (L748)
...
- `_pi05_phase(name, **fields) -> ContextManager[None]` — Adapter-local shortcut for `phase_timer(name, prefix="pi05", gpu_mb=True, log=_log)`. (L489)
...
- `_resolve_pretrained_path(spec, repo_id) -> str` — Returns a local directory containing the lerobot processor sidecars. Local path → verbatim; otherwise routes through `_processors.resolve_processor_dir`. (L503)

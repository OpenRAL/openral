...
- `_check_rskill_compatibility(env_cfg) -> RSkillManifest | None` — Load the rSkill manifest, run `rSkill.check_compatibility` against the registered `RobotDescription`, return the manifest or `None` for built-in mock policies. Strict-by-construction; also runs `_check_policy_units_owned`. (L1010)
- `_check_policy_units_owned(manifest, robot, scene_id) -> None` — `ROSConfigError` when `PolicyIOCodec.from_manifest(manifest, robot)` is non-identity (degrees / `gripper_scale` / joint permutation) and `SCENES.meta(scene_id)["converts_policy_units"]` is not `True` — `sim run` applies no codec, so only a converting scene (`so101_box`, `tabletop_push`, `openarm_tabletop_pnp`) may take such a checkpoint. Fail-loud guard, never a conversion. (L941)
- `_SEQUENTIAL_INIT_ENV: str = "OPENRAL_SIM_SEQUENTIAL_INIT"` — Module-level constant: the env var that forces `_build_env_and_policy` onto the legacy sequential path (set to `"1"`). (L1110)
...
- `_build_env_and_policy(env_cfg, policy_cfg=None) -> (SimRollout, PolicyAdapter)` — `make_env` sees `env_cfg`, `make_policy` sees `policy_cfg` (slot-keyed scene cameras) when given. Builds env + policy concurrently on a 2-worker `ThreadPoolExecutor` by default; sequential when `OPENRAL_SIM_SEQUENTIAL_INIT=1`. Logs `sim_init_parallel`/`sim_init_sequential` timing. Exceptions from either side propagate verbatim. (L1160)
- `_seed_global_rngs(seed) -> None` — Seed Python / NumPy / Torch RNGs so stochastic policies reproduce per `(seed + episode_idx)`. (L1253)
- `_open_viewer_and_pacing(env, env_cfg, *, strict_view) -> (Any, float | None)` — Opens a passive `mujoco.viewer` against the adapter's `mujoco_handles()`, sets the camera + geom visibility via `_aim_viewer_camera`, and computes the per-step sleep budget so the viewer renders at the env's natural sim-time. (L1302)
- `_aim_viewer_camera(viewer, env, mj_model, mj_data) -> None` — Sets the viewer's opening free-camera pose + geom visibility (hides collision shells so textures render); the user can still orbit/zoom afterward. Best effort — failure logs and leaves MuJoCo's default camera. (L1364)
...
- `const _VIEW_ENV = 'OPENRAL_SIM_VIEW'` (L1118)
- `_scene_requires_sequential_init(env_cfg) -> bool` — `SCENES.meta(scene_id)["sequential_init"]`: scenes whose backend construction races the policy build on parallel threads (`openarm_tabletop_pnp`, `tabletop_push`, `maniskill3`, `simpler_env`, `robocasa*`). (L1153)
...
- `_strip_hf_uri(uri, *, field_name) -> str` (L145) — Strip the `hf://` prefix off a manifest URI, validating it is present. (L332)

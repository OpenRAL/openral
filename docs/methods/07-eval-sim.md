...
- `const _PACKING_TOKENS: frozenset[str] = frozenset({'nf4', 'int8'})` — The two tokens for which `QuantPlan.quantize` is `True`. (L979)
- `const _QUANT_TOKEN_ALIASES: dict[str, str]` — Dtype-spelling normalization table consumed by `canonical_quant_token` (`int4`/`4bit` → `nf4`, float aliases fold, …). (L986)
...
- `resolve_weights_files(repo_id, *, revision=None) -> list[str]` (L640) — Local paths of the weights for `repo_id`: `[model.safetensors]`, or every shard named by a sharded `model.safetensors.index.json` (`save_pretrained`'s layout above its shard size). Accepts the **directory** `resolve_rskill_to_hf_with_revision` returns for an `openral rskill install`-ed snapshot or a bare Hub repo id (cached-first download of the single file, falling back to the index + shards on `EntryNotFoundError`); forwards `revision` so weights and `config.json` share one pin. The one place that tells directory from repo id; `peek_safetensors_keys` and `stream_state_into_policy` route through it.
- `const WEIGHTS_FILE = "model.safetensors"` — The single-file checkpoint layout. (L624)
- `const WEIGHTS_INDEX_FILE = "model.safetensors.index.json"` — The sharded layout's index; its `weight_map` names each tensor's shard. (L627)
- `BufferRebuilder = Callable[[str, Any, Any], bool]` / `KeyMap = Callable[[list[str]], dict[str, str]]` — The two family hooks of the meta-init loaders: rebuild a family-only non-persistent buffer in place (`(name, owning_module, buffer) -> handled`), and map a checkpoint's raw keys to `{parameter_name: source_key}` (the family's own `from_pretrained` key normalisation). (L749)
- `rebuild_non_persistent_buffers(policy, *, torch, family, rebuild=None) -> None` — Rebuilds every buffer a meta-initialised (`init_empty_weights` + `to_empty`) policy cannot load from its checkpoint: `*.position_ids` (int64 arange) generically, anything else through the family's `rebuild` hook. A non-persistent buffer nobody rebuilt is refused with `ROSRuntimeError` naming `family` instead of running as garbage — π0.5's `embed_scale` left uninitialised gave cosine 0.03 against `from_pretrained` with every loaded tensor matching (Thor, 2026-09-23). Persistence read from each module's `_non_persistent_buffers_set` (a pre-`.to(cuda)` `Linear8bitLt` cannot `state_dict()`). (L783)
- `expand_covered_keys_via_tied_storage(policy, covered_keys) -> set[str]` — Detects tied parameters via `Tensor.untyped_storage().data_ptr()` and extends `covered_keys` to include every key in a tied group whenever any member is already covered. Uses `named_parameters(remove_duplicate=False)` because the default dedups tied params away. Moved out of `pi05.py`. (L832)
- `stream_state_into_policy(policy, repo_id, *, torch, family, revision=None, key_map=None) -> None` — The meta-init substitute for a family's `from_pretrained` weight load: reads each tensor of a (possibly sharded) checkpoint on the CPU and `copy_`-es it into the already-materialised parameter / persistent buffer (moves + casts; one model plus one tensor peak, no source-dtype temporary on the GPU). `key_map` applies the family's key normalisation first. **Fails closed** with `ROSConfigError`: a mapped key naming no slot (*unexpected*) or a parameter / persistent buffer nothing was copied into (*missing*, it would run on its reset value) — the only allowed miss is a tied alias sharing storage with a loaded slot. Source keys the `key_map` drops on purpose are counted `dropped` in the `meta_init_state_streamed` log. Used by π0.5's bf16 (on the GPU) and int8 (on the CPU, before the `Int8Params` pack) paths. (L857)
- `targeted_reset_parameters(policy, *, covered_keys) -> None` — Calls `module.reset_parameters()` only on modules whose params are NOT covered by the about-to-load state dict (`covered_keys=None` reproduces the unconditional reset). Model-agnostic, shared by π0.5, MolmoAct2, and future meta-init families. (L1250)
- `tie_transformers_weights(policy) -> None` — Calls `module.tie_weights()` on each outermost transformers backbone (pre-order, skipping already-tied descendants); a raising `tie_weights` is non-fatal. Shared by π0.5 and MolmoAct2. (L1310)
- `normalise_manifest_dtype(manifest) -> str | None` — Pulls `manifest.quantization.dtype.value` as a string; returns `None` for manifests without a quantization block. Lifted out of `pi05.py` into the shared module so smolvla / xvla / future quantized adapters share one implementation. (L950)
- `QUANTIZATION_DTYPE_ENV: str` — `"OPENRAL_QUANTIZATION_DTYPE"` — the one all-family per-run dtype override, replacing the old per-family `OPENRAL_GR00T_QUANTIZATION`/`OPENRAL_RLDX_QUANTIZATION` env vars. (L974)
- `canonical_quant_token(raw) -> str | None` — Normalises a dtype spelling to the one token every consumer understands (`int4`/`4bit` → `nf4`, float aliases fold, unknown tokens pass through lowercased). `None` stays `None` so “unset” is distinguishable from an explicit `"none"` (= do not quantize). (L1003)
- `QuantPlan(dtype, quantize, source, extra)` — `NamedTuple` returned by `resolve_quant_plan`. `quantize` is `True` only for a packing token (`nf4` / `int8`), which is the single definition of "does this load rewrite Linears"; `source` is `env` / `spec_quantization` / `spec_extra` / `manifest` / `default` / `unset` so the adapter can log what won; `extra` carries the manifest's `quantization.extra` knobs. (L1027)
- `quantization_extra(manifest) -> Mapping[str, Any]` — Returns `manifest.quantization.extra` (the `quantize_scope` / `nf4_min_params` knobs), or `{}`. One home for the packing knobs, which used to sit in `policy_extras` next to unrelated adapter settings — which is why each family grew its own reader. (L1049)

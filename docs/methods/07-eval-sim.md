...
- `targeted_reset_parameters(policy, *, covered_keys) -> None` — Calls `module.reset_parameters()` only on modules whose params are NOT covered by the about-to-load state dict (`covered_keys=None` reproduces the unconditional reset). Model-agnostic, shared by π0.5, MolmoAct2, and future meta-init families. (L922)
- `tie_transformers_weights(policy) -> None` — Calls `module.tie_weights()` on each outermost transformers backbone (pre-order, skipping already-tied descendants); a raising `tie_weights` is non-fatal. Shared by π0.5 and MolmoAct2. (L982)
...
- `default_dtype_for_device(device) -> str` — Picks a default load dtype when the manifest doesn't specify one: `nf4` on CUDA (so 3.4 B-param backbones fit in ~4 GiB), `fp32` elsewhere. Lifted out of `pi05.py`. (L910)

...
- `class _Registry(Generic[T])` — Tiny ID → factory map. (L44)
  - `__init__(kind)` (L55)
...
- `SimRunner.activate() -> None` — Validates the manifest, builds env + policy concurrently, arms the first reset-tick, opens the outer `sim.run` OTel span. (L418)
- `SimRunner.deactivate() -> None` — Flush a trailing episode if any, close the viewer / policy / env, close the outer span. Idempotent. (L515)
- `SimRunner._should_terminate() -> bool` — Returns True once `n_episodes` `EpisodeResult`s have been emitted. (L552)
- `SimRunner._tick_impl(tick_idx) -> TickResult` — Dispatch reset-tick vs step-tick. (L600)
- `SimRunner._reset_tick(tick_idx) -> TickResult` — env.reset + policy.reset; `action_applied=False`, `inference_ms=0.0`, `step_idx=None`. (L610)
- `SimRunner._step_tick(tick_idx) -> TickResult` — policy.step + env.step; populates `step_idx`, `reward`, `terminated`, `truncated`, `action_applied=True`. (L664)
- `SimRunner._finalize_episode() -> None` — Build an `EpisodeResult` from the per-step `_EpisodeBuffer`, append to `episode_results`, reset the buffer. (L866)
...
- `const _IMAGE_NDIM = 3` (L66)
- `const _VIDEO_FRAME_CAP = 8192` (L65)
- `const _GRAYSCALE_CHANNELS = 1` (L67)
- `const _RGB_CHANNELS = 3` (L68)
...
- `_write_debug_videos(args, results, env_cfg) -> None` — Render the two-band debug MP4(s) via `openral_sim._video.save_episode_mp4`. (L834)
...
- `class RoboCasaBackendOptions(BaseModel)` — The backend-owned `options_model` for `SceneSpec.backend_options` on `robocasa` and `robocasa/gr1` (moved out of `openral_core`); prebuilt-vs-procedural XOR, and range-checks scene-pool pins against the upstream domain so an out-of-domain pin fails with a clear error instead of a bare `KeyError` from RoboCasa's arena builder. `state_layout` defaults to `None` (follow the rSkill's `state_contract`). (L131)
...
- `_check_robocasa_scene_ids(value, *, field, id_min, id_max, group_ids) -> list[int] | int | None` — Rejects a layout/style id outside the upstream domain. (L81)
- `prop _ROBOCASA_LAYOUT_ID_MIN, _ROBOCASA_LAYOUT_ID_MAX, _ROBOCASA_LAYOUT_GROUP_IDS, _ROBOCASA_STYLE_ID_MIN, _ROBOCASA_STYLE_ID_MAX, _ROBOCASA_STYLE_GROUP_IDS, _ROBOCASA_LAYOUT_AND_STYLE_SHORTHANDS, _ROBOCASA_LAYOUT_AND_STYLE_PAIR_LEN` — The upstream RoboCasa domain `RoboCasaBackendOptions` range-checks scene-pool pins against. (L66–78)
- `_validate_backend_options(scene) -> RoboCasaBackendOptions` — `SCENES.validate_options` narrowed to the RoboCasa model. (L1895)
...
- `_resolve_state_layout(env_cfg, opts) -> str` — Obs `state_layout`: an explicit `backend_options.state_layout` wins; else the rSkill manifest's `state_contract.layout` mapped via `_MANIFEST_TO_ROBOCASA_LAYOUT` (`rc365`→`human300_16d`, `human300_16d`, `smolvla_9d`, `gr1`); else `_DEFAULT_STATE_LAYOUT` (`human300_16d`). One scene YAML serves every rSkill.
...
- `read_panda_mobile_base_velocity(model, data) -> NDArray[np.float32]` — Returns body-frame `(vx, vy, wz)` for the robosuite OmronMobileBase, de-rotated by the live yaw. Returns `zeros(3)` when the base joints aren't in this model (no-op for non-PandaMobile envs). (L1640)
- `synthesize_laser_scan_2d(*, model, data, base_body_id=None, n_beams=360, max_range_m=12.0, laser_height_m=0.30) -> NDArray[np.float32]` — Single-origin 2D laser fan from the panda_mobile base via one `mj_ray` per beam (not the batched `mj_multiRay`, which can't re-cast past a self-hit). Ranges are clamped to `max_range_m`, never NaN/inf, so a Nav2 costmap can't be poisoned. Self-excludes the chassis body. (L1719)
- `head_cam_enabled() -> bool` — True iff `OPENRAL_ROBOCASA_HEAD_CAM` is set, gating the synthetic forward nav camera so a manipulation run pays for no extra render. `deploy sim`/`run` derive it automatically from the palette; an explicit env value still wins. (L1527)
- `render_head_view(renderer, model, data, base_joint_names, *, height_m=1.30, forward_offset_m=0.42) -> NDArray[np.uint8] | None` — Renders the forward egocentric `head` frame from the robot.yaml `head` sensor's geometry. `None` for non-mobile-base models. Consumed by the InternVLA-N1 VLN rSkill via `observation.images.head`. (L1547)
...
- `sim_time_ns() -> int | None` (method on `_RoboCasaSim`) — `round(MjData.time * 1e9)` off `mujoco_handles()`. RoboCasa rewinds the clock on `reset`, so it is monotonic only within an episode — `SimAttachedHAL.sim_time_ns` adds the cross-reset offset. (L1057)
- `task_success() -> bool | None` (method on `_RoboCasaSim`) — The env's own task-success predicate (robosuite/RoboCasa `_check_success()`), read on demand and side-effect free. Live, not latched — an undone task reads `False` again. `None` means no predicate and must never be read as failure. Not consulted during `deploy sim`'s continuous stepping; `SimAttachedHAL` polls it instead. (L842)
...
- Constants `_OMRON_BASE_JOINT_NAMES`, `_OMRON_BASE_JOINT_NAMES_FALLBACK`, `_LASER_DEFAULT_N_BEAMS=360`, `_LASER_DEFAULT_MAX_RANGE_M=12.0`. (L1484)
...
- `env.py`:
  ...
  - `_build_openarm_tabletop_scene(env_cfg) -> _OpenArmTabletopRollout` — Scene factory registered as `SCENES.register("openarm_tabletop_pnp", fixed_robot="openarm", sequential_init=True, sim_clock=True, base_pose=True, converts_policy_units=True)(_build_openarm_tabletop_scene)`. (env.py L762)
...
- `_assets.py`:
  - `class BoxSceneOptions(BaseModel)` — Frozen, `extra="forbid"` model holding every scene-geometry knob (arena, robot mount, two cameras, block + tube dimensions, spawn ranges, insertion thresholds, `control_hz`, LeRobot joint-units affine; `joint_signs` entries must be ±1). The scene's registered `options_model`. (_assets.py L46)
  - `compose_so101_box_mjcf(options=None, robot_description=None) -> tuple[str, Path]` — Reads the robot's MJCF, re-anchors its `base` body to `options.robot_base_xyz`, splices a wrist camera into the `gripper` body, and appends the arena/walls/light/overhead camera/block/tube to the worldbody. The `base`/`gripper` anchors are fixed MJCF body names, so `deploy sim` composes the twin off the robot's own MJCF. (_assets.py L543)
  ...
  - `const _UNSET: tuple[float, ...] = ()` — Sentinel default distinguishing "not overridden" from an explicit empty range in `BoxSceneOptions`. (`_assets.py` L40)
- `env.py`:
  - `_options_from_backend_options(scene) -> BoxSceneOptions` — `SCENES.validate_options` narrowed to `BoxSceneOptions`; unknown keys / wrong shapes raise `ROSConfigError`. (env.py L69)
  ...
  - `build_so101_box_scene(env_cfg) -> _So101BoxRollout` — Scene factory registered as `SCENES.register("so101_box", fixed_robot="so101_follower", options_model=BoxSceneOptions, sim_clock=True, converts_policy_units=True)(build_so101_box_scene)`. Composes the MJCF, resolves the 6 arm actuators by their upstream numeric names (`"1"`…`"6"`), and caches the block / tube / hole / tip site indices for the success check. (env.py L492)
...
- `_assets.py`:
  - `class TabletopOptions(BaseModel)` — Frozen, `extra="forbid"` model holding every scene-geometry knob (table slab, robot-mount fallback, cube, goal disc + radius, two world cameras, opt-in wrist camera, settle steps, optional reset joint pose, scaled joint-unit affine, lighting). The scene's registered `options_model`. (_assets.py L51)
  - `compose_tabletop_mjcf(description, options=None, *, base_pose=None) -> mujoco.MjModel` — Resolves the robot MJCF from the manifest, re-anchors the robot root body to `base_pose` (or the yaw-only `robot_base_xyz` fallback), appends the table/cube/goal/cameras/light, and compiles. Robot-agnostic — no body-name regex. (_assets.py L228)
  ...
  - `infer_wrist_camera_mount_body(description) -> str | None` — Return the wrist camera's MJCF parent body from the robot manifest. (`_assets.py` L191)
- `env.py`:
  - `_options_from_backend_options(scene) -> TabletopOptions` — `SCENES.validate_options` narrowed to `TabletopOptions`; unknown keys / wrong shapes raise `ROSConfigError`. (env.py L60)
  ...
  - `build_tabletop_push_scene(env_cfg) -> _TabletopPushRollout` — Scene factory registered as `SCENES.register("tabletop_push", options_model=TabletopOptions, sequential_init=True, sim_clock=True, base_pose=True, converts_policy_units=True)(build_tabletop_push_scene)` (free-axis). Composes the model, resolves the robot's actuator→joint transmissions for state read + action clipping, and caches the cube body/freejoint + goal site indices. Raises `ROSConfigError` when `robot_id` has no registered manifest. (env.py L358)
  ...
  - `const _SPAWN_ATTEMPTS = 32` (`env.py` L57)
...
- `const _PACKING_TOKENS: frozenset[str] = frozenset({'nf4', 'int8'})` — The two tokens for which `QuantPlan.quantize` is `True`. (L978)
- `const _QUANT_TOKEN_ALIASES: dict[str, str]` — Dtype-spelling normalization table consumed by `canonical_quant_token` (`int4`/`4bit` → `nf4`, float aliases fold, …). (L985)
...
- `install_prequantized_linears(policy, state, *, device, torch) -> tuple[int, set[str]]` — Replaces every `Linear4bit.weight` with `Params4bit.from_prequantized(...)` data read from `state`. Returns `(n_modules_rebuilt, consumed_state_keys)` so `overlay_prequantized_state` skips the consumed keys when it copies the residual. (L295)
- `overlay_prequantized_state(policy, state, *, device, torch, family, source) -> None` — Loads an nf4 prequant pack completely or refuses: `install_prequantized_linears`, then `copy_` of every other key, then the `stream_state_into_policy` coverage check. A parameter/persistent buffer left unfilled (not a tied alias of a filled one) or a key naming no slot raises `ROSConfigError` naming them. (L374)
- `detect_prequantized_nf4(spec) -> str | None` — Probes the rSkill's HF repo for a `quantization_metadata.json` sentinel; returns the repo id when the pack is present, `None` otherwise. Routed through `hf_download_cached_first` so a cache hit avoids the HEAD request. (L446)
- `load_prequantized_state_for_rskill(policy, spec, *, torch, log_event_prefix="rskill") -> None` — Combined entry point: validates the metadata sentinel, downloads `model.safetensors`, hands the pack to `overlay_prequantized_state`, which loads it completely or raises `ROSConfigError` (no fallback: the fast meta-init path has no other weight source). No-op when the rSkill ships bf16 weights — adapters can call it unconditionally after their own `quantize_nf4_in_place`. (L511)
- `peek_safetensors_keys(repo_id, *, revision=None) -> set[str] | None` — Reads only the safetensors header(s) (every shard of a sharded checkpoint, via `resolve_weights_files`) and returns the raw key set, for both prequantized (nf4) and bare (int8/bf16) checkpoints. Used by `targeted_reset_parameters` to skip the init walk for modules whose params the upcoming state load will overwrite anyway; an adapter whose loader renames keys (π0.5) maps the set itself. (L692)
- `resolve_weights_files(repo_id, *, revision=None) -> list[str]` (L639) — Local paths of the weights for `repo_id`: `[model.safetensors]`, or every shard named by a sharded `model.safetensors.index.json` (`save_pretrained`'s layout above its shard size). Accepts the **directory** `resolve_rskill_to_hf_with_revision` returns for an `openral rskill install`-ed snapshot or a bare Hub repo id (cached-first download of the single file, falling back to the index + shards on `EntryNotFoundError`); forwards `revision` so weights and `config.json` share one pin. The one place that tells directory from repo id; `peek_safetensors_keys` and `stream_state_into_policy` route through it.
- `const WEIGHTS_FILE = "model.safetensors"` — The single-file checkpoint layout. (L623)
- `const WEIGHTS_INDEX_FILE = "model.safetensors.index.json"` — The sharded layout's index; its `weight_map` names each tensor's shard. (L626)
- `BufferRebuilder = Callable[[str, Any, Any], bool]` / `KeyMap = Callable[[list[str]], dict[str, str]]` — The two family hooks of the meta-init loaders: rebuild a family-only non-persistent buffer in place (`(name, owning_module, buffer) -> handled`), and map a checkpoint's raw keys to `{parameter_name: source_key}` (the family's own `from_pretrained` key normalisation). (L748)
- `rebuild_non_persistent_buffers(policy, *, torch, family, rebuild=None) -> None` — Rebuilds every buffer a meta-initialised (`init_empty_weights` + `to_empty`) policy cannot load from its checkpoint: `*.position_ids` (int64 arange) generically, anything else through the family's `rebuild` hook. A non-persistent buffer nobody rebuilt is refused with `ROSRuntimeError` naming `family` instead of running as garbage — π0.5's `embed_scale` left uninitialised gave cosine 0.03 against `from_pretrained` with every loaded tensor matching (Thor, 2026-09-23). Persistence read from each module's `_non_persistent_buffers_set` (a pre-`.to(cuda)` `Linear8bitLt` cannot `state_dict()`). (L782)
- `expand_covered_keys_via_tied_storage(policy, covered_keys) -> set[str]` — Detects tied parameters via `Tensor.untyped_storage().data_ptr()` and extends `covered_keys` to include every key in a tied group whenever any member is already covered. Uses `named_parameters(remove_duplicate=False)` because the default dedups tied params away. Moved out of `pi05.py`. (L831)
- `stream_state_into_policy(policy, repo_id, *, torch, family, revision=None, key_map=None) -> None` — The meta-init substitute for a family's `from_pretrained` weight load: reads each tensor of a (possibly sharded) checkpoint on the CPU and `copy_`-es it into the already-materialised parameter / persistent buffer (moves + casts; one model plus one tensor peak, no source-dtype temporary on the GPU). `key_map` applies the family's key normalisation first. **Fails closed** with `ROSConfigError`: a mapped key naming no slot (*unexpected*) or a parameter / persistent buffer nothing was copied into (*missing*, it would run on its reset value) — the only allowed miss is a tied alias sharing storage with a loaded slot. Source keys the `key_map` drops on purpose are counted `dropped` in the `meta_init_state_streamed` log. Used by π0.5's bf16 (on the GPU) and int8 (on the CPU, before the `Int8Params` pack) paths. (L856)
- `targeted_reset_parameters(policy, *, covered_keys) -> None` — Calls `module.reset_parameters()` only on modules whose params are NOT covered by the about-to-load state dict (`covered_keys=None` reproduces the unconditional reset). Model-agnostic, shared by π0.5, MolmoAct2, and future meta-init families. (L1249)
- `tie_transformers_weights(policy) -> None` — Calls `module.tie_weights()` on each outermost transformers backbone (pre-order, skipping already-tied descendants); a raising `tie_weights` is non-fatal. Shared by π0.5 and MolmoAct2. (L1309)
- `normalise_manifest_dtype(manifest) -> str | None` — Pulls `manifest.quantization.dtype.value` as a string; returns `None` for manifests without a quantization block. Lifted out of `pi05.py` into the shared module so smolvla / xvla / future quantized adapters share one implementation. (L949)
- `QUANTIZATION_DTYPE_ENV: str` — `"OPENRAL_QUANTIZATION_DTYPE"` — the one all-family per-run dtype override, replacing the old per-family `OPENRAL_GR00T_QUANTIZATION`/`OPENRAL_RLDX_QUANTIZATION` env vars. (L973)
- `canonical_quant_token(raw) -> str | None` — Normalises a dtype spelling to the one token every consumer understands (`int4`/`4bit` → `nf4`, float aliases fold, unknown tokens pass through lowercased). `None` stays `None` so “unset” is distinguishable from an explicit `"none"` (= do not quantize). (L1002)
- `QuantPlan(dtype, quantize, source, extra)` — `NamedTuple` returned by `resolve_quant_plan`. `quantize` is `True` only for a packing token (`nf4` / `int8`), which is the single definition of "does this load rewrite Linears"; `source` is `env` / `spec_quantization` / `spec_extra` / `manifest` / `default` / `unset` so the adapter can log what won; `extra` carries the manifest's `quantization.extra` knobs. (L1026)
- `quantization_extra(manifest) -> Mapping[str, Any]` — Returns `manifest.quantization.extra` (the `quantize_scope` / `nf4_min_params` knobs), or `{}`. One home for the packing knobs, which used to sit in `policy_extras` next to unrelated adapter settings — which is why each family grew its own reader. (L1048)
...
- `_pi05_phase(name, **fields) -> ContextManager[None]` — Adapter-local shortcut for `phase_timer(name, prefix="pi05", gpu_mb=True, log=_log)`. (L385)
...
- `_lerobot_key_map(policy) -> KeyMap` — π0.5's `KeyMap` hook: exactly the renames lerobot 0.6 `PI05Policy.from_pretrained` applies — the policy's own `_fix_pytorch_state_dict_keys` (openpi layout: `action_time_mlp_*` → `time_mlp_*`, `state_proj.*` and adaRMS-mismatched norm weights dropped, `lm_head` also written to `embed_tokens`) called with the key names as values (`_SourceKey`, a `str` with a no-op `clone`), then the `model.` prefix. Before it, the fast paths loaded raw keys and only logged an INFO miss count, so an openpi-layout checkpoint ran on reset-initialised parameters. (L364)
- **bf16 fast meta-init** (`bf16_fast_meta_init`: plain bf16 on CUDA) — the policy is built on meta, allocated straight on the GPU by `to_empty(device)`, weights tied, non-covered params reset, buffers rebuilt (`_init_pi05_buffers`), and the checkpoint streamed in by the shared `stream_state_into_policy` with `_lerobot_key_map` — one model plus one tensor peak, no CPU materialisation. lerobot's `from_pretrained` built the 3.6 B graph on the CPU first: 347 s idle / 666 s under a live graph on a Jetson AGX Orin (no native bf16) against a 0.1 s mmap of the file (2026-09-22). Parity against lerobot's load semantics (every parameter + buffer bit-exact, and a forward): `tests/sim/test_pi05_meta_init_parity.py` (bf16 needs a ≥ 10 GiB GPU; int8 runs on 8 GiB).
- `_rebuild_gemma_buffer(name, mod, buf, *, torch) -> bool` — π0.5's `BufferRebuilder` hook: RoPE `*.inv_freq` / `*.original_inv_freq` (theta from the module's `config.rope_parameters`, else its `rope_theta` / `base`, else 10000) and Gemma's `*.embed_scale` (`sqrt(hidden_size)`, from the module's surviving `scalar_embed_scale`). (L306)
- `_init_pi05_buffers(policy, *, torch) -> None` — `rebuild_non_persistent_buffers(..., family="pi05", rebuild=_rebuild_gemma_buffer)`; shared by the nf4, int8 and bf16 fast meta-init paths. (L344)
- `_rebuild_int8_params_for_linear8bitlt(policy) -> int` — Re-wraps each `Linear8bitLt.weight` as a fresh `bnb.nn.Int8Params(has_fp16_weights=False)`. `to_empty(device=...)` strips `Parameter` subclasses; without this rewrap the downstream `policy.to(<cuda>)` would never trigger bnb's int8 pack. (L261)
- `_resolve_pretrained_path(spec, repo_id) -> str` — Returns a local directory containing the lerobot processor sidecars. Local path → verbatim; otherwise routes through `_processors.resolve_processor_dir`. (L399)
...
- `_build_molmoact2(env_cfg) -> _MolmoAct2Adapter` (L406, `@POLICIES.register("molmoact2")`) — Loads Ai2's MolmoAct2 (~5.49B params, Molmo2-ER VLM + flow-matching action expert). Resolves manifest + dtype, delegates to `_load_molmoact2_model`, then wires replay cadence (clamped to `max_action_horizon`), norm tag, image flips, state/action dims, and autocast. (L745)
- `_checkpoint_camera_order(camera_keys, image_keys, checkpoint_keys) -> tuple[str, ...]` — `predict_action` takes images positionally, so the manifest aliases decide which slot is which view: orders the slots by the checkpoint's `norm_stats.json` `camera_keys` (matching each slot's `checkpoint_image_keys`); an empty checkpoint order keeps slot order; a checkpoint camera with no aliased slot raises `ROSConfigError`. (L245)
- `_norm_stats_camera_keys(repo_id, revision, norm_tag) -> tuple[str, ...]` — The checkpoint's camera order for `norm_tag` from `norm_stats.json` (the file `predict_action` fetches anyway; offline when cached). (L280)
- `_load_molmoact2_model(*, torch, model_cls, config_cls, processor_cls, source_repo, spec, device, dtype_str, max_crops) -> tuple[model, processor, use_nf4, torch_dtype]` (L382) — Always loads the processor. With a prequant NF4 pack on CUDA, builds the model on the meta device, quantizes, then loads the prequant state directly — skipping lerobot's slow bf16 `from_pretrained` materialisation. Falls back to the slow bf16-load-then-quantize path for bf16/non-CUDA/no-pack. nf4 is CUDA-only and the default (bf16 needs ~11 GiB and OOMs an 8 GiB GPU; nf4 needs ~4 GiB). (L575)
- `_resolve_max_crops(spec, manifest) -> int | None` (L305) — Resolves the image-processor `max_crops` override: `vla.extra` → env var → manifest → `None` (checkpoint default 8). A secondary lever only — the actual 8 GiB fit is enabled by `_enable_expandable_segments`, not this. (L548)
- `_enable_expandable_segments() -> None` (L131) — Sets the CUDA allocator's `expandable_segments:True` before the first allocation, called when `device` is CUDA. Without it, MolmoAct2 NF4's first-forward embedding `cat` OOMs an 8 GiB card; with it, it fits. No-op if the operator already set the var. (L135)
- `_molmoact2_phase(name, **fields) -> ContextManager[None]` (L105) — Adapter-local shortcut for `phase_timer(name, prefix="molmoact2", gpu_mb=True, log=_log)`. (L161)
- `_import_molmoact2() -> tuple[Any, Any, Any]` (L118) — Imports lerobot's in-tree `MolmoAct2ForConditionalGeneration` + `MolmoAct2Config` + transformers `AutoProcessor` behind a typed `ROSConfigError` install hint. Returns `Any` (lerobot / transformers are optional, unstubbed deps). (L174)
- `_strip_hf_uri(uri, *, field_name) -> str` (L145) — Strip the `hf://` prefix off a manifest URI, validating it is present. (L338)
...
- `_RLDX_LAYOUT_VIDEO_KEYS` / `_resolve_video_keys(layout, image_preprocessing, camera_keys) -> tuple[str, ...]` — Per-layout sidecar video keys (the wire format; `_RLDX_LAYOUT_CAMERA_COUNT` derives from it) and the slot→video-key resolution: an aliased slot takes `input_template.format(cam=alias)` (in-tree rldx manifests state `input_template: "video.{cam}"` + the layout keys explicitly), an un-aliased slot the layout key at its position. (L302)
...
- `_build_rldx(env_cfg) -> _RLDXSidecarAdapter` — `@POLICIES.register("rldx")` factory. Honours `OPENRAL_RLDX_*` env overrides and `vla.extra` (`image_size`/`timeout_ms`/`auto_spawn`/`boot_timeout_s`/`embodiment_tag`/`model_id`). Cameras are slots from `resolve_camera_keys` (default: the manifest's slots), their wire keys from `_resolve_video_keys`. Quantization comes from `resolve_quant_plan` (default `nf4`) → `sidecar_quant_token` over the sidecar's {none,nf4,int8}; dispatches the obs/action contract via `_resolve_state_layout` and the port via `_resolve_sidecar_port`.
...
- `_build_gr00t_b1k(env_cfg) -> PolicyAdapter` — `@POLICIES.register("gr00t_b1k")` factory: merges the manifest's `policy_extras` under `spec.extra` and delegates to `behavior_groot.build_behavior_groot_policy` (which resolves quantization via `sidecar_quant_token` over {none,nf4,int8}). (L423)
- `_build_gr00t(env_cfg) -> _GrootAdapter` — `@POLICIES.register("gr00t")` factory. Resolves the rSkill manifest, `snapshot_download`s the raw N1.7 checkpoint locally (the GR00T processor factory only recognises a local raw-checkpoint dir), builds the config, loads `GrootPolicy.from_pretrained` on CPU under a float32 default dtype, NF4-rewrites the backbone, then a pure `policy.to(device)` packs the `Linear4bit` shells onto the GPU. Reads `OPENRAL_GR00T_EMBODIMENT_TAG` env + `vla.extra` (`embodiment_tag` default `libero_sim`); GR00T video keys are `checkpoint_image_keys` over the manifest's slot-keyed `image_preprocessing.aliases` (the `policy_extras.image_modality_keys` reader was removed; `embodiment_tag` stays an extra — it selects the modality config, it is not a rename). Quantization comes from the shared `resolve_quant_plan` (default `nf4`) guarded by `require_supported_dtype` over {nf4, fp32, none} (an unpacked load runs fp32 params — `model_params_fp32=not quantize` — so bf16/fp16/int8 raise); the manifests declare `int4` (what runs) with `quantization.extra.stored_dtype: bf16`; `quantize_scope` reads from `quantization.extra`. Execution horizon (`_replan_steps`) comes from `resolve_n_action_steps` (manifest / `--n-action-steps`), defaulting to the policy's `_action_queue_steps` — GR00T's `libero_sim` 8-of-16. (L453)
...
- `_LingBotVla2Adapter(spec, device, _client, _camera_keys, _replan_steps, ...)` — `PolicyAdapter` proxying the sidecar. Predicts a `(50, 14)` chunk and replays one 14-D step per `step`, refilling when the queue drains (`_DEFAULT_REPLAN_STEPS=25`, mirroring the upstream deploy default). `_refill` marshals the 3 camera slots (keyed by `_image_keys`, from the manifest aliases) + proprio state + instruction to `get_action`. (L230)
- `_build_lingbot(env_cfg, *, variant) -> _LingBotVla2Adapter` — Shared factory behind an auto-managed `SidecarClient`. `variant` selects the model family (`v2` 6B Qwen3-VL MoE / `v1` 4B Qwen2.5-VL dense expert); reads `vla.extra` for model/camera/port overrides; quantization via `resolve_quant_plan` → `sidecar_quant_token` over {none,nf4} (the per-family `OPENRAL_LINGBOT_VLA*_QUANTIZATION` / `vla.extra.quantization` knobs were removed). `expected_identity={model,robo_name}` guards against adopting a stale sidecar on the default port. (L310)
- `_build_lingbot_vla2(env_cfg)` / `_build_lingbot_vla(env_cfg)` — `@POLICIES.register("lingbot_vla2")` (6B) and `@POLICIES.register("lingbot_vla")` (4B RoboTwin post-train, `model_family: "lingbot_vla"` in `rskills/lingbot-vla-4b-robotwin/rskill.yaml`) thin wrappers over `_build_lingbot`. (L428)
- `_policy_default_port(model_id, robo_name, variant="v2") -> int` / `_resolve_camera_io(manifest, extra, scene_cameras) -> tuple[slots, keys]` / `_resolve_model_id(spec, extra, default_model_id) -> str` / `_locate_sidecar_script() -> Path` / `_opt_int(value, default) -> int` — per-(variant, model, embodiment) SHA-256-bucketed default port (20000–39999); camera slots (`resolve_camera_keys`, default the manifest's slots) renamed to the sidecar views by the manifest's `image_preprocessing` (`input_template: "{cam}"` + slot-keyed aliases), rejected unless they map onto exactly `cam_high`/`cam_left_wrist`/`cam_right_wrist` (the `vla.extra.camera_keys` reader was removed); model-id resolution (`vla.extra.model_id` > manifest `weights_uri` > `spec.weights_uri` > per-variant default); boot-helper locator; `vla.extra` int coercion. (L130)

...
- `_load_bf16_state_for_int8(policy, repo_id, *, torch) -> None` — Downloads `<repo>/model.safetensors` via `resolve_weights_file` (directory-aware) and applies it via `policy.load_state_dict(strict=False)`. The int8 fast meta-init path's substitute for lerobot's ~152 s `PI05Policy.from_pretrained`. (L329)

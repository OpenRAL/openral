...
- `const _PACKING_TOKENS: frozenset[str] = frozenset({'nf4', 'int8'})` — The two tokens for which `QuantPlan.quantize` is `True`. (L686)
- `const _QUANT_TOKEN_ALIASES: dict[str, str]` — Dtype-spelling normalization table consumed by `canonical_quant_token` (`int4`/`4bit` → `nf4`, float aliases fold, …). (L693)
...
- `peek_safetensors_keys(repo_id, *, filename="model.safetensors") -> set[str] | None` — Reads only the safetensors header and returns its key set, for both prequantized (nf4) and bare (int8) checkpoints. Used by `targeted_reset_parameters` to skip the init walk for modules whose params the upcoming state load will overwrite anyway. (L606)
- `resolve_weights_file(repo_id, *, filename="model.safetensors", revision=None) -> str` (L571) — Forwards `revision` (the adapter's pinned Hub revision) on the Hub path so weights and `config.json` share one pin. Local path of the weights for `repo_id`, whether that is the **directory** `resolve_rskill_to_hf_with_revision` returns for an `openral rskill install`-ed snapshot (file taken from the directory) or a bare Hub repo id (cached-first download). The one place that tells the two apart; `peek_safetensors_keys`, `_load_bf16_state_for_int8` and `_stream_bf16_state_to_device` all route through it — every direct `hf_hub_download` broke on the directory case.
- `targeted_reset_parameters(policy, *, covered_keys) -> None` — Calls `module.reset_parameters()` only on modules whose params are NOT covered by the about-to-load state dict (`covered_keys=None` reproduces the unconditional reset). Model-agnostic, shared by π0.5, MolmoAct2, and future meta-init families. (L957)
- `tie_transformers_weights(policy) -> None` — Calls `module.tie_weights()` on each outermost transformers backbone (pre-order, skipping already-tied descendants); a raising `tie_weights` is non-fatal. Shared by π0.5 and MolmoAct2. (L1017)
- `normalise_manifest_dtype(manifest) -> str | None` — Pulls `manifest.quantization.dtype.value` as a string; returns `None` for manifests without a quantization block. Lifted out of `pi05.py` into the shared module so smolvla / xvla / future quantized adapters share one implementation. (L657)
- `QUANTIZATION_DTYPE_ENV: str` — `"OPENRAL_QUANTIZATION_DTYPE"` — the one all-family per-run dtype override, replacing the old per-family `OPENRAL_GR00T_QUANTIZATION`/`OPENRAL_RLDX_QUANTIZATION` env vars. (L681)
- `canonical_quant_token(raw) -> str | None` — Normalises a dtype spelling to the one token every consumer understands (`int4`/`4bit` → `nf4`, float aliases fold, unknown tokens pass through lowercased). `None` stays `None` so “unset” is distinguishable from an explicit `"none"` (= do not quantize). (L710)
- `QuantPlan(dtype, quantize, source, extra)` — `NamedTuple` returned by `resolve_quant_plan`. `quantize` is `True` only for a packing token (`nf4` / `int8`), which is the single definition of "does this load rewrite Linears"; `source` is `env` / `spec_quantization` / `spec_extra` / `manifest` / `default` / `unset` so the adapter can log what won; `extra` carries the manifest's `quantization.extra` knobs. (L734)
- `quantization_extra(manifest) -> Mapping[str, Any]` — Returns `manifest.quantization.extra` (the `quantize_scope` / `nf4_min_params` knobs), or `{}`. One home for the packing knobs, which used to sit in `policy_extras` next to unrelated adapter settings — which is why each family grew its own reader. (L756)
...
- `_pi05_phase(name, **fields) -> ContextManager[None]` — Adapter-local shortcut for `phase_timer(name, prefix="pi05", gpu_mb=True, log=_log)`. (L504)
...
- `_expand_covered_keys_via_tied_storage(policy, covered_keys) -> set[str]` — Detects tied parameters via `Tensor.untyped_storage().data_ptr()` and extends `covered_keys` to include every key in a tied group whenever any member is already covered. Uses `named_parameters(remove_duplicate=False)` because the default dedups tied params away. (L256)
- `_load_bf16_state_for_int8(policy, repo_id, *, torch, revision=None) -> None` — Downloads `<repo>/model.safetensors` via `resolve_weights_file` (directory-aware) and applies it via `policy.load_state_dict(strict=False)`. The int8 fast meta-init path's substitute for lerobot's ~152 s `PI05Policy.from_pretrained`. (L454)
- `_stream_bf16_state_to_device(policy, repo_id, *, device, torch, revision=None) -> None` — Reads each tensor on the CPU and `copy_`-es it into its device-side target (no source-dtype temporary on the GPU); `named_parameters(remove_duplicate=False)` keeps every tied alias name. The **bf16 fast meta-init** path (`bf16_fast_meta_init`: plain bf16 on CUDA): the policy is built on meta, allocated straight on the GPU by `to_empty(device)`, weights tied, non-covered params reset, buffers rebuilt, and each safetensors tensor is read with `safe_open(..., device=device)` and `copy_`-ed into its parameter — one model plus one tensor peak, no CPU materialisation. lerobot's `from_pretrained` built the 3.6 B graph on the CPU first: 347 s idle / 666 s under a live graph on a Jetson AGX Orin (no native bf16) against a 0.1 s mmap of the file (2026-09-22). Logs `pi05_bf16_state_streamed` with loaded / missing / unexpected counts.
- `_init_rope_and_position_buffers(policy, *, torch) -> None` — An `embed_scale` on a module with neither `scalar_embed_scale` nor `embedding_dim` is refused by name like any other unknown buffer. Rebuilds every NON-persistent buffer a meta-initialised (`init_empty_weights` + `to_empty`) policy cannot load from safetensors, the way its module builds it: `*.position_ids` (int64 arange), RoPE `*.inv_freq` / `*.original_inv_freq` (theta from the module's `config.rope_parameters`, else its `rope_theta` / `base`, else 10000), and Gemma's `*.embed_scale` (`sqrt(hidden_size)`, from the module's surviving `scalar_embed_scale`). Any other non-persistent buffer is refused with `ROSRuntimeError` instead of running as garbage — `embed_scale` left uninitialised gave cosine 0.03 against `from_pretrained` with all 813 loaded tensors matching (Thor, 2026-09-23). Shared by the nf4, int8 and bf16 fast meta-init paths.
- `_rebuild_int8_params_for_linear8bitlt(policy) -> int` — Re-wraps each `Linear8bitLt.weight` as a fresh `bnb.nn.Int8Params(has_fp16_weights=False)`. `to_empty(device=...)` strips `Parameter` subclasses; without this rewrap the downstream `policy.to(<cuda>)` would never trigger bnb's int8 pack. (L289)
- `_resolve_pretrained_path(spec, repo_id) -> str` — Returns a local directory containing the lerobot processor sidecars. Local path → verbatim; otherwise routes through `_processors.resolve_processor_dir`. (L518)

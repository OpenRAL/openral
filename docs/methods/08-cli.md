...
- `_resolve_clock_origin(*, hal_mode, config, pinned=None) -> str` (L453) — The graph's clock authority: a scene's `runtime.clock_origin` wins; else `host_wall` for `deploy run`, and for `deploy sim` `simulation` when the scene's backend exposes a clock (every scene-configured bare twin, and scene-attached backends registered with `sim_clock=True`), `host_wall` otherwise — including a scene-less `--robot`-only twin. Raises `ROSConfigError` for a `simulation` pin with no clock.

...
- `_resolve_clock_origin(*, hal_mode, config, pinned=None) -> str` (L443) — The graph's clock authority: a scene's `runtime.clock_origin` wins; else `host_wall` for `deploy run` and `simulation` for a sim backend that exposes a clock (every bare twin). Raises `ROSConfigError` for a `simulation` pin with no clock.

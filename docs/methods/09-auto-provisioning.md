...
- const `_CUDA_HOME_NVCC: Path` — Canonical "currently selected CUDA toolkit" symlink (`/usr/local/cuda/bin/nvcc`) preferred by `_probe_cuda_toolkit_version` over `$PATH`. (gpu.py L611)
...
- `_probe_jetson(warnings, *, model_path=None, release_path=None) -> JetsonInfo | None` — Probe a Tegra host. `model_path` / `release_path` accept fixtures for unit tests; production reads `/proc/device-tree/model` + `/etc/nv_tegra_release`. Returns `None` + warning when the board is unknown. (gpu.py L470)
- const `_DEFAULT_MODEL_PATH: Path` — Default `/proc/device-tree/model` path probed by `_probe_jetson`; overridable for tests. (gpu.py L466)
- const `_DEFAULT_RELEASE_PATH: Path` — Default `/etc/nv_tegra_release` path probed by `_probe_jetson`; overridable for tests. (gpu.py L467)

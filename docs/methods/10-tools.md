...
- `--transition-timeout-s` — per-transition spin budget, **default `300.0`** (`lifecycle_autostart.py:161`). The launch file passes `"300.0"` as a literal for the HAL (`sim_e2e.launch.py:1112`); the kernel gets 120 s and the reasoner 300 s. There is **no** `openral deploy sim` flag, launch argument, or `DeployRuntime` field that overrides it.
- **Known gap — sidecar boot can exceed the bound.** The bound covers everything a scene factory does inside `on_configure`, including a sidecar's spawn + `_wait_for_boot`. Those backends carry their own, larger budgets: `isaac_sim` 900 s, `behavior` 1200 s, `robotwin` 600 s (`rlbench` 300 s, and it passes the constant directly so `backend_options.boot_timeout_s` is silently ignored). All four `scenes/deploy/isaac_*.yaml` plus `scenes/deploy/behavior_r1pro.yaml` explicitly set `boot_timeout_s: 1200` — 4× what the launcher will wait for, so those five deploy scenes declare a budget the launcher cannot honour. On expiry `_drive_transition` raises and the autostart exits non-zero **while `on_configure` keeps running**, which can leave the HAL parked in INACTIVE with nothing left to drive ACTIVATE and no message naming the cause. Provisioning was moved out of the callback (`SCENES.register(provision=…)`); the *boot* was not. Unmeasured: no in-tree record of a real warm Isaac / OmniGibson boot duration, so whether 300 s is actually exceeded in practice is undetermined.

...
- `_enum_schema(cls) -> dict[str, Any]` — Minimal JSON Schema for a `str` Enum. (L132)
- `export_schemas(out_dir=_OUT_DIR) -> dict[str, Any]` — Export JSON Schema for every public model. (L144)
- `check_drift(out_dir=_OUT_DIR) -> bool` — On-disk schemas == regenerated. (L197)

...
- `build_serve_argv(*, vllm_bin, model, host, port, tool_call_parser, max_model_len, gpu_memory_utilization, enforce_eager, served_model_name=None) -> list[str]` — the `vllm serve` argv. `--max-model-len` (default 8192) holds the reasoner prompt + tools; `--enforce-eager` + `--gpu-memory-utilization` (default 0.90, `$OPENRAL_COSMOS3_GPU_MEM_UTIL`) are the 8 GB-fit knobs.
- `main() -> int` — argparse (`--model` default `nvidia/Cosmos3-Edge`, `--host`, `--port` default 8901, `--tool-call-parser`, `--max-model-len`, `--gpu-memory-utilization`, `--no-enforce-eager`, `--home`, `--venv`); strips `PYTHONPATH`/`PYTHONHOME`, resolves the served view, and `os.execvpe`s into `vllm serve`.

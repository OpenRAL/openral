...
_Boot helper for the NVIDIA Cosmos 3 reasoner server, companion to `openral_reasoner.cosmos3.Cosmos3ToolUseClient` (`PROVIDER=cosmos`)._ Provisions an isolated Python 3.12 venv (`OPENRAL_COSMOS3_SIDECAR_VENV` to reuse one) from the pure-PyPI hash-pinned `tools/sidecar_requirements/cosmos3_reasoner.lock` (vllm≥0.23 — the first line serving the Cosmos 3 reasoner tower), then `os.execvpe`s into `vllm serve` with tool calling enabled (`--enable-auto-tool-choice --tool-call-parser hermes` — the Cosmos 3 reasoner follows Qwen3-VL-compatible message conventions). No ZMQ server script: vLLM's OpenAI-compatible HTTP API *is* the provider contract the reasoner already speaks. Out-of-process for the same dependency/VRAM isolation reasons as `qwen_vlm_sidecar.py`. Cosmos 3 weights are OpenMDW-1.1 (commercial OK) — no license guard.

- `ensure_venv(home, *, override=None) -> Path` — return the sidecar venv python, provisioning + installing the pinned lock if absent (sentinel-guarded); honours `$OPENRAL_COSMOS3_SIDECAR_VENV`.
- `build_serve_argv(*, vllm_bin, model, host, port, tool_call_parser, max_model_len) -> list[str]` — the `vllm serve` argv; `--max-model-len` (default 32768) caps the KV cache well below the model's 256K window for edge GPUs.
- `main() -> int` — argparse (`--model` default `nvidia/Cosmos3-Edge`, `--host`, `--port` default 8901, `--tool-call-parser`, `--max-model-len`, `--home`, `--venv`); strips `PYTHONPATH`/`PYTHONHOME` and `os.execvpe`s into `vllm serve`.

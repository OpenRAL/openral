...
- `_enum_schema(cls) -> dict[str, Any]` — Minimal JSON Schema for a `str` Enum. (L151)
- `export_schemas(out_dir=_OUT_DIR) -> dict[str, Any]` — Export JSON Schema for every public model. (L163)
- `check_drift(out_dir=_OUT_DIR) -> bool` — On-disk schemas == regenerated. (L215)

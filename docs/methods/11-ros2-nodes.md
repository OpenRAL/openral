...
- `compose_so100_runtime(*, skill_resolver=None) -> ComposedRuntime` (compose.py L330) — SO-100 convenience wrapper over `compose_runtime`.
- `ComposedRuntime` (compose.py L40) — Dataclass returned by `compose_runtime`: `description`, `aggregator`, `world_state_node`, `skill_runner_node`, `slam_bridge`, `world_cloud_bridge`, `dataset_recorder_bridge` fields.
...
- `generate_launch_description() -> LaunchDescription` (L2576) — Robot-agnostic deploy-sim launch graph entry point; wraps `compose_runtime_graph` in an `OpaqueFunction`.

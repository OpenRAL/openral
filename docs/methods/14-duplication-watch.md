...
- **Per-rig perception caps — deliberate C++/Pydantic mirror, update in lockstep.** `DeployRuntime._check_perception_caps` + the fields' JSON Schema `maximum` / defaults (`python/core/src/openral_core/schemas.py`) vs. `kMaxWorldVoxelDeadlineMs` / `kMaxWorldVoxelDataAgeBudgetMs` / `kDefaultWorldVoxelDataAgeBudgetMs` (`cpp/openral_safety_kernel/include/openral_safety_kernel/lifecycle_kernel.hpp`), `kMaxOctreeAgeS` (`packages/openral_octomap_bridge/include/openral_octomap_bridge/octree_freshness.hpp`) and `kMaxSelfFilterPaddingM` (`.../self_filter.hpp`). Kept apart because a node launched without `openral deploy` never sees the schema, and the Layer-2 bridge must not depend on the Layer-5 kernel. Each side fails closed on its own; drift means a node looser than the schema (a hole) or tighter (a validated scene refused at configure). `tests/unit/test_perception_caps_mirror.py` pins every pair.

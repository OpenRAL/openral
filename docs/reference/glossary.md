...
- **rSkill** — A packaged, signed, capability-tagged unit of robot behavior. One HF Hub repo. Loaded by `rSkill.from_pretrained(...)`.
- **HAL** — Hardware Abstraction Layer. The `openral.hal.HAL` Protocol; per-robot adapters live in `packages/openral_hal_<robot>/`.

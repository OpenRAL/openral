...
| ALOHA bimanual (gym-aloha) | [`robots/aloha_bimanual/`](https://github.com/OpenRAL/openral/tree/master/robots/aloha_bimanual/) | `AlohaMujocoHAL` (`MujocoArmHAL`, bimanual) + real-HW `AlohaHAL` over Interbotix XS | ✓ sim · HW adapter landed, never validated — no physical ALOHA rig (`tests/hil/test_aloha.py` is gated on a `lab-aloha` runner that does not exist) |
...
| Enactic OpenArm v2 bimanual | [`robots/openarm/`](https://github.com/OpenRAL/openral/tree/master/robots/openarm/) | `OpenArmMujocoHAL` (`MujocoArmHAL`, 16-DoF bimanual) + real-HW `OpenArmRealHAL` (four `openarm_bringup` ros2_control controllers over Damiao CAN FD) | ✓ sim · ✓ HW — CAN transport + motor round-trip pass on the rig (`tests/hil/test_openarm_can_live.py`); `send_action` verified moving the arm. Run by hand on a lab host: no CI runner, and no in-tree real-HW deploy scene, so no end-to-end policy rollout on the arm |

...
| ACT SO-101 pen | SO-101 real / sim | `so101_follower` | TBD | wrist + overhead, both trained at **640×480**; ACT does **not** resize (no resize step in `modeling_act.py`; preprocessor is normalization-only) | Yes (in ckpt) | `rskills/act-so101-pen/` | Apache-2.0 | ACT pen checkpoint; optional whole-model ONNX/TRT (`OPENRAL_ACT_TRT`). Manifest pins `camera1` ≥640×480; `camera2` ≥256×256 only because the `wrist` rig's declared intrinsics are 256² — that view runs below its training resolution |

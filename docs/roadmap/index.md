...
| Geometric safety + watchdog | 🟡 in flight — chunk-rate safety pass-through + envelope checks (✅); deadman/E-stop forwarders + human-estop ship as lifecycle nodes (`openral_safety_watchdog` / `openral_human_estop`) but nothing launches them — [#290](https://github.com/OpenRAL/openral/pull/290) puts them on the deploy graph; self/world/voxel collision + OctoMap→voxel bridge in dev |

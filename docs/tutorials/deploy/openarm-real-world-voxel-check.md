...
1. **The arm sees itself — now filtered, with a blind shell to accept.** The sim HAL's
   ...
   primitives at the cloud's capture stamp (joint states from the scene's
   `joint_states_topic`, camera pose from tf2) and removes every return within
   `_SELF_FILTER_PADDING_M` (5 cm) of them, plus a held payload's primitives once an
   ...
   The cost is a 5 cm shell around the arm in which the kernel cannot see a real obstacle

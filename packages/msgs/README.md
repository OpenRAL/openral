...
| File | Role |
| --- | --- |
| `ActionChunk.msg` | A bounded action chunk (length × DoF) emitted by an S1 Skill. Mirrors `openral_core.Action`; optional `cartesian_delta_scale` converts controller-native deltas to physical metres/radians for predictive safety without changing raw actuator bytes. **ADR-0102:** optional `joint_names` names the joints a slot-dispatched `JOINT_*` chunk targets — such a chunk is zero-padded to full dof for the kernel's `n_dof` check, so without the names it cannot say which joints it owns (and `0.0` is a legal target). Empty = a whole-vector action in `RobotDescription.joints` order, which is every pre-0102 producer's meaning. `runner_session_id` (uint64) is the rSkill runner process's random id: `tick_index` restarts with every runner process, so the HAL keys its replay watermark on `(runner_session_id, tick_index)` and refuses a superseded session; `0` = unknown (legacy producer, tick-only heuristic). |
...
| `PlaceDeclaration.msg` | Dispatch's typed statement that a place phase is active for a payload, and which target it is being placed into (ADR-0097). Published on `/openral/place_declaration` by the `ExecuteRskill` action server and consumed by the sim attachment-evidence producer in the HAL. It exempts **nothing** on its own — it only makes support contact *on the declared target* attestable as a place-phase `SupportContactWitness`, under the identical bounds, fail-closed rules and hysteresis the pick witness is under. Scoped to one goal: retracted (`active=false`) on goal end, cancel and E-stop, and expired by every consumer on its own `timeout_s` so a dead dispatcher cannot leave an exemption armed. Its optional `region_valid` / `region` pair (ADR-0097's 2026-08-14 amendment) is filled in by the *producer*, never by dispatch, and carries the bounded approach allowance described under `PlaceRegion.msg`. That "never" is enforced rather than only documented: `openral_core.DeployScene` refuses a scene-supplied region, the rSkill runner drops any region on the declaration it publishes (goal or scene), and the sim producer replaces or clears the field with its own measurement — so a region the kernel sees was always measured by whoever claims to have measured it (HZ-0097-2/4). |
| `SegmentMasks.msg` | **Diagnostic-only** mirror of the latest `srv/SegmentInView` reply on `/openral/perception/masks` — `header` (the caller's attach stamp + the camera optical frame), `camera` (a `SensorSpec` name), `rskill_id`, the same full-frame `mono8` `sensor_msgs/Image[]` candidates in area-ascending order, and the parallel advisory scores. Published by `openral_perception_ros.segmenter_node` only when its `publish_debug_masks` parameter is set (default **false**), at sensor-class QoS (`BEST_EFFORT` / `VOLATILE` / `KEEP_LAST=1`). Consumed by the dashboard's `PerceptionOverlaySubscriber` to draw masks on the camera tiles. **Nothing in the safety or attachment path reads it** — the HAL producer uses its own service reply — so dropping every message on this topic changes no robot behaviour. |
...
| `SegmentInView.srv` | HAL's vision attachment-evidence producer asking a promptable segmenter (`kind: "segmenter"`, SAM 2.1) which wrist-frame pixels are the grasped payload. Geometric prompt (a 3-D TCP point, not a text query, not a pixel) in; raw **mono8** `sensor_msgs/Image[]` candidate masks out. Plural because `multimask` yields nested subpart/part/whole hypotheses and only the HAL, which holds the depth frame, can pick between them — the segmenter node must not. `mask_scores_advisory` is parallel to `masks`, recorded and never thresholded nor used to select. |
...
| --- | --- | --- |
| `msg/ActionChunk` | `openral_core.Action` | `control_mode` enum mirrors `ControlMode`; empty `cartesian_delta_scale` means identity for backward compatibility; `runner_session_id` 0 decodes as `Action.runner_session_id = 0` (legacy). |
...
| `msg/FailureTrigger` | `openral_core.FailureEvidence` (discriminated union) | `kind` ∈ {`KIND_TIMEOUT`=0, `KIND_FORCE`=1, `KIND_WORKSPACE`=2, `KIND_PERCEPTION`=3, `KIND_CRITIC`=4, `KIND_CONTROLLER`=5, `KIND_SELFVERIFY`=6, `KIND_HUMAN`=7, `KIND_WAM`=8, `KIND_REASONER_TIMEOUT`=9, `KIND_COLLISION`=10, `KIND_SUPPRESSED_SUMMARY`=254}; `severity` ∈ {`SEVERITY_INFO`=0, `WARN`=1, `FAIL`=2, `ABORT`=3}. `evidence_json` decodes via `pydantic.TypeAdapter(FailureEvidence).validate_json(...)`. This hard-breaks the wire format (no migrator, no string-shaped fallback). Both constant blocks are mirrored as plain ints in `openral_observability.failure_bus` for ROS-free callers; `tests/unit/test_failure_bus_idl_mirror.py` pins that mirror against these numbers in both directions. |
...
| `msg/SegmentMasks` | none (ROS-only, diagnostic) | Deliberately has no Pydantic counterpart: it is a live re-publication of a `srv/SegmentInView` reply for display, not a persisted artifact, so `tools/schema_export.py` has nothing to drift against. Field-for-field a subset of that service's response (plus `camera` / `rskill_id` / a `Header`), and it must stay that way — a second mask shape would be a second thing the dashboard has to decode. |
| `srv/SegmentInView` | `openral_core.SegmenterContract` (bounds the request) / `openral_core.AttachedCollisionObject` (what the caller builds from the reply) | Masks travel as raw `mono8` `sensor_msgs/Image`, not run-length encoded: a one-shot attach/detach/regrasp call is ~307 kB per candidate and at most three, and a bespoke codec in the safety-adjacent path buys bytes at the cost of a hand-synchronised encoder/decoder pair. `masks` and `mask_scores_advisory` are parallel arrays; the scores are trace-only — the caller selects among candidates, and gates them, on geometry (CLAUDE.md §1.1). |

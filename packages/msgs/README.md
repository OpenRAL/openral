...
| `srv/SegmentInView` | `openral_core.SegmenterContract` (bounds the request) / `openral_core.AttachedCollisionObject` (what the caller builds from the reply) | The mask travels as a raw `mono8` `sensor_msgs/Image`, not run-length encoded: a one-shot attach/detach/regrasp call is ~307 kB, and a bespoke codec in the safety-adjacent path buys bytes at the cost of a hand-synchronised encoder/decoder pair. `mask_score_advisory` is trace-only — the caller's gates are geometric (CLAUDE.md §1.1). |

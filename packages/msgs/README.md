...
| `msg/PlaceDeclaration` | `openral_core.PlaceDeclaration` | `is_live(now_ns=...)` is the consumer-side liveness rule (active **and** inside `timeout_s`); `timeout_s` is capped at `PlaceDeclaration.MAX_TIMEOUT_S` (600 s — strictly above the largest in-tree `latency_budget.max_execution_s`, so no legitimate goal is cut short and nothing mission-scale is accepted). |
...
| `action/ExecuteRskill` | `openral_core.RSkillManifest` (referenced via `rskill_id`), `openral_core.PlaceDeclaration` (goal's `place_declaration`) | The action server lives in `openral_rskill_ros`. |

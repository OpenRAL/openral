...
_GYM_ALOHA_AVAILABLE = importlib.util.find_spec("gym_aloha") is not None

from openral_hal import AlohaMujocoHAL
...
def test_lifecycle_transitions_full_cycle() -> None:
    ...
    rclpy.init()
    try:
        node = _HALLifecycleNode(node_name=NODE_NAME, hal_factory=AlohaMujocoHAL)
        try:

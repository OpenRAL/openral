"""Lifecycle smoke test for ``openral_hal_g1``.
...
the real ``G1MujocoHAL`` factory. The HAL pulls its MJCF lazily (typically
from ``robot_descriptions``) and ships its canonical
:class:`openral_core.RobotDescription` (``G1_DESCRIPTION``).
...
from openral_hal import G1MujocoHAL
...
def test_lifecycle_transitions_full_cycle() -> None:
    ...
    rclpy.init()
    try:
        node = _HALLifecycleNode(node_name=NODE_NAME, hal_factory=G1MujocoHAL)
        try:

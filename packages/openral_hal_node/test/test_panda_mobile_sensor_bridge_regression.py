...
Run::
    ...
    MUJOCO_GL=egl uv run pytest \\
        packages/openral_hal_node/test/test_panda_mobile_sensor_bridge_regression.py -v \\
        --timeout=600

Per CLAUDE.md §1.11: no mocks. Real ``ManifestHALLifecycleNode``, real robocasa
...
def test_panda_mobile_robocasa_sensor_bridge_regression() -> None:
    ...
    Brings up the generic ``ManifestHALLifecycleNode`` with

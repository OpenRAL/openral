"""HAL OTel-span coverage for a real-HAL adapter behind the generic node (dashboard).

Drives the real ``ManifestHALLifecycleNode`` (the one node ``openral_hal_node``
ships) against a real

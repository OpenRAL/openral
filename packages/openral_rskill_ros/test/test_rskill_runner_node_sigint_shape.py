...
_NODE = (
    _REPO_ROOT / "packages" / "openral_rskill_ros" / "openral_rskill_ros" / "rskill_runner_node.py"
)


def _parse() -> ast.Module:
    """Parse the rskill_runner_node module as Python. Fail loudly if missing."""
    assert _NODE.is_file(), f"rskill_runner_node not found at {_NODE}"
...
def test_imports_external_shutdown_exception() -> None:
    ...
    assert "ExternalShutdownException" in imported, (
        f"rskill_runner_node must import ExternalShutdownException from "
...
def test_no_bare_rclpy_shutdown_call() -> None:
    ...
    assert not bare_calls, (
        f"rskill_runner_node must use rclpy.try_shutdown() (idempotent); "
...
def test_uses_try_shutdown() -> None:
    ...
    assert try_shutdown_lines, (
        "rskill_runner_node must call rclpy.try_shutdown() on the teardown path."
    )
...
def test_spin_wrapped_in_sigint_except() -> None:
    ...
    for node in ast.walk(tree):
        ...
        for stmt in node.body:
            ...
            break
    assert spin_trys, (
        "no `try:` block wraps a `.spin()` call in rskill_runner_node — the "

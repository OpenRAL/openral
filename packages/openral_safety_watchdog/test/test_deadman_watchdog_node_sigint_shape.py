...
§5 bullet 4). The structural contract additionally proves the ``except``
...
energised: by the time ``main()`` is exiting, the watchdog has already fired
its estop on ``/openral/estop`` and the C++ safety kernel owns the actuation
gate independently.
"""

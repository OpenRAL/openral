...
| `object_depth_points_topic` | `""` (disabled) | Depth-fallback amendment (#11) — depth `PointCloud2` used as the lift's depth source when no fresh voxel grid exists (e.g. `--no-enable-octomap`). Empty disables the fallback. `deploy_e2e.launch.py` passes the robot manifest's first depth sensor (`openral_core.camera_topic(<name>, CameraTopicKind.POINTS)`), or empty when the robot declares none (ADR-0108: consumers derive, never guess). |

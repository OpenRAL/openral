...
* A preview image/video (``![](…)``, ``<img>``, ``<video>``) — **warning**
  only, so pre-existing skills without one keep passing.

Manifest checks (beyond the Pydantic schema):

* ``description`` isn't the scaffold placeholder and is
  ``>= _MIN_DESCRIPTION_LENGTH`` chars.
* At least one of ``paper_url`` / ``source_repo`` is set.
* ``weights_uri`` / ``source_repo`` contain no ``TEMPLATE`` sentinel.
"""
...
A README containing
``<!-- openral:rskill-readme-delegates-to: <relative-path> -->`` is a stub:
section checks run against the resolved sibling instead, though placeholder
and length checks still apply to the stub. Single-hop only — a delegated-to
README cannot itself delegate further.

Used by the RLDX-1 ports (``rldx1-ft-rc365-nf4`` delegates shared
architecture/license docs to ``rldx1-ft-libero-nf4``) to avoid duplicating
content across sibling skills.
"""

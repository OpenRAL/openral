...
# lerobot.datasets requires the `datasets` extra (HuggingFace datasets
# library). The bare `lerobot` package installs without it; we check
# explicitly so the module skips rather than fails at runtime.
pytest.importorskip(
    "datasets",
    reason=(
        "'datasets' package not installed — required by lerobot.datasets; "
        "install via `uv pip install 'lerobot[dataset]'`"
    ),
)


@pytest.fixture
def _write_traced_dataset(
    _zero_frame: Callable[[RobotDescription], tuple[np.ndarray, dict[str, np.ndarray], np.ndarray]],
) -> Callable[[Path, RobotDescription], list[tuple[str, str]]]:
    ...
    return _write_traced_dataset


def test_read_frame_trace_returns_producing_tick_ids(
    so100_robot: RobotDescription,
    tmp_path: Path,
    _write_traced_dataset: Callable[[Path, RobotDescription], list[tuple[str, str]]],
) -> None:

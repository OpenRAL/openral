...
applied automatically by ``openral deploy run``). The one thing detection
*cannot* know is the **task** (what job the robot should do). This module turns
the detected :class:`RobotDescription` into a schema-valid
:class:`RobotEnvironment` with the known fields filled and the task left as an
obvious ``TODO`` placeholder for the operator to edit before
``openral deploy run``. The rSkill that drives the robot is **not** pinned in
the config — the reasoner selects it at runtime from the installed registry.

The boundary is deliberate (CLAUDE.md §1.2, §1.4): we never invent a task
instruction, because a wrong one would silently drive real motors.
"""
...
class ScaffoldOverrides(BaseModel):
    ...
    ``TODO`` sentinel for ``task`` and ``None`` for ``safety``. These are
    exactly the things detection *cannot* infer (CLAUDE.md §1.2, §1.4), so
...
# Sentinel placeholder values. Self-documenting and schema-valid; the CLI
# greps for them to flag the still-TODO task before deploy.
...
def scaffold_robot_environment(
    ...
    ``/dev/video*`` device path when known) — and leaves ``task`` as a ``TODO``
    placeholder the operator must edit. ``safety`` is left ``None`` so the
    robot's own :attr:`RobotDescription.safety` envelope (the robot limits)
    applies unchanged. No rSkill is pinned — the reasoner picks it at runtime.
    ...
    Args:
        ...
        overrides: Optional operator answers (:class:`ScaffoldOverrides`) that
            replace the ``TODO`` task sentinel, set a ``safety`` workspace box,
            and/or attach a ``metadata.label``. Anything left unset keeps the
            placeholder so the deploy guard still flags it.

    Returns:
        A validated :class:`RobotEnvironment`. The ``task`` stays a ``TODO``
        placeholder unless ``overrides`` filled it, so it loads and round-trips
        but is **not runnable** until ``task`` is real.
    ...
    metadata["edit_before_deploy"] = ["task"] if task.id == TODO_TASK_ID else []

...
def test_panda_mobile_front_cameras_are_liftable_wrist_is_not() -> None:
    """RGB cameras with a dedicated *_optical_frame get a TF; a link-framed one does not.

    panda_mobile's ``front_left`` / ``front_right`` (per ADR-0069) declare
    ...
    assert "front_left" in liftable
    assert "front_right" in liftable

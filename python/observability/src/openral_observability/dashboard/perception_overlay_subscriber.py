...
This module closes that gap for the detector half, which streams today:
`openral_msgs/PromptStamped` on `/openral/perception/objects`, carrying an
`openral_core.ObjectsMetadata` JSON document. It follows
...
advisory *display* only — nothing here is a safety input.

The segmenter half is service-only upstream (`openral_msgs/srv/SegmentInView`
returns mono8 masks one-shot per attach event, it does not stream), so there is
no topic to subscribe to yet and this module deliberately does not invent one.
What it does ship is the decode seam that side needs —
:func:`mono8_mask_to_png_b64`, written against that service's exact mono8
convention — plus `TelemetryStore.set_perception_masks`, so the moment a mask
publisher exists the overlay lights up without a frontend change.
"""
...
class PerceptionOverlaySubscriber:
    """Feeds the store's camera overlays from `/openral/perception/objects`."""
    ...
    def __init__(self, store: TelemetryStore) -> None:
        ...
        Args:
            store: The dashboard's telemetry store; every decoded detection set
                ...
                :meth:`TelemetryStore.set_perception_detections`.
        """
        self._store = store
        self._node: Any = None

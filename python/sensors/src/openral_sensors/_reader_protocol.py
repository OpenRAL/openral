...
The ``openral_sensors.ros_publisher.SensorRosPublisher`` accepts any
...
class SensorReaderLike(Protocol):
    """Structural alias of ``openral_runner.sensor_reader.SensorReader``.
    ...
    def read_latest(self, max_age_ms: int | None = None) -> SensorFrame:
        """Return the most recent buffered ``SensorFrame``. Non-blocking."""

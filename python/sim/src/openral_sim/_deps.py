...
_ROBOCASA_GR1_RUNTIME_DEPS = tuple(
    dep for dep in _ROBOCASA_RUNTIME_DEPS if dep != "robosuite.examples"
)
...
def _has_robocasa_gr1() -> bool:
    ...
    if not (
        ...
        return False
    if not all(_has_module(m) for m in _ROBOCASA_GR1_RUNTIME_DEPS):
        return False

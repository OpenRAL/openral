...
Consolidates three things :mod:`openral_sim.policies.pi05`,
:mod:`openral_sim.policies.rldx`, and :mod:`openral_sim.policies.smolvla`
each need on their ``_build_*``:

1. **Manifest resolution** (:func:`load_manifest_for_spec`) — pull the
   :class:`RSkillManifest` from ``spec.weights_uri`` when it's a bare
...
2. **Lazy torch + lerobot import** (:func:`lazy_import_lerobot`) — SmolVLA
...
:mod:`openral_sim._quantization` for the generic dtype/bnb helpers.
"""

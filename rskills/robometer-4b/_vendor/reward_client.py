...
def main() -> int:
    ...
    print(
        f"[client] {len(vr)} src frames @ {src_fps:.1f}fps -> {len(frames)} sampled @ ~{args.fps}fps"
    )
    ...
    for start in range(0, len(frames), args.chunk):
        ...
        if q.get("stale") and not q.get("frames_seen"):
            continue
        print(
            f"{q['frames_seen']:>7} | {q['progress_now']:>12.4f} | {q['success_now']:>11.4f} "
            f"| {q['progress_trend']:>10.4f} | {q['stalled']!s:>7}"
        )

...
| `heavy-lanes.yml` | PR with the `heavy-lanes` label (`opened`/`synchronize`/`reopened`/`labeled`/`unlabeled`), `workflow_dispatch` | The opt-in dependency lanes (`sim`, `libero`, `robocasa`, …) for the targets `tools/select_tests.py` selects, one matrix job per lane. | **On request only.** `select` → `lane` → `heavy-lanes` (gate + ledger attest). Without the label every job is skipped (not red). |

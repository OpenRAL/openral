...
class TestUR5eDownstreamEStop:
    def test_zz_estop_stops_the_scaled_controller_and_the_pendant_program(
        ...
        node, transport = _production_stop_seam("openral_hil_ur5e_stop", ur5e_hal)
        try:
            with pytest.raises(ROSEStopRequested, match="downstream stop acknowledged"):
                ur5e_hal.estop()
            report = ur5e_hal.last_stop_report
            ...
            assert ur5e_hal.estop_recovery is EStopRecovery.RESTART_REQUIRED

...
_OPENARM_SCENE = Path(__file__).resolve().parents[2] / "scenes" / "deploy" / "openarm_tabletop.yaml"
...
def _lifecycle_harness() -> Iterator[tuple[Any, Any, dict[str, list[Any]]]]:
    ...
    composition = DeployScene.from_yaml(str(_OPENARM_SCENE)).composition
    ...
    node.set_parameters(
        [
            ...
            rclpy.parameter.Parameter(
                "scene_composition_json", value=composition.model_dump_json()
            ),
        ],
    )
    ...
    helper.create_subscription(
        RosJointState,
        "/openral_hal_openarm/joint_states",
...
def test_on_safe_action_emits_hal_send_action_span(
    ...
    with _lifecycle_harness() as (executor, _node, observed):
        ...
        pub = pub_node.create_publisher(ActionChunk, "/openral/safe_action", chunk_qos)
        # Wait for the HAL's /openral/safe_action subscriber to discover this
        # publisher before publishing; VOLATILE durability drops messages sent
        # before the DDS match completes (observed flake on loaded CI hosts).
        discovery_deadline = time.monotonic() + 2.0
        while pub.get_subscription_count() < 1 and time.monotonic() < discovery_deadline:
            executor.spin_once(timeout_sec=0.02)
        chunk = ActionChunk()
        chunk.n_dof = 16
        chunk.horizon = 1
...
def test_estop_latch_blocks_subsequent_safe_action(
    captured_spans: InMemorySpanExporter,
) -> None:
    ...
    with _lifecycle_harness() as (executor, node, observed):
        ...
        # Wait for DDS discovery before publishing; VOLATILE durability drops
        # messages sent before the subscriber match completes.
        discovery_deadline = time.monotonic() + 2.0
        while (
            estop_pub.get_subscription_count() < 1 or action_pub.get_subscription_count() < 1

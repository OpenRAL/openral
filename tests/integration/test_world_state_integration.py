    ...
    *,
    ...
    node.set_parameters(
        [
            ...
            rclpy.parameter.Parameter("joint_states_topic", value="/test_world_state/joint_states"),
    ...
    joint_pub = helper.create_publisher(RosJointState, "/test_world_state/joint_states", 1)
    fast_msgs: list[Any] = []
    slow_msgs: list[Any] = []
    ws_qos = QoSProfile(
...
def test_high_load_snapshot_consistency() -> None:
    ...
    with _lifecycle_harness(
        ...
        # rclpy publishers are not thread-safe; protect concurrent publish
        # calls with a lock so the DDS layer never sees a torn message (which
        # can produce a partially-serialised JointState with len(position)==1
        # instead of the expected 0 or 6).

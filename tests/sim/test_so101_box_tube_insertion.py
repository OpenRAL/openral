...
def test_so101_box_wrist_camera_is_top_mounted_and_link_aligned(env_cfg) -> None:
    """The wrist camera sits on top of the gripper and points link-forward."""
    from openral_sim import SCENES
    ...
    np.testing.assert_allclose(cam_pos_local, np.array([0.02, 0.0, -0.02]), atol=2e-3)
    assert float(np.dot(cam_forward_local, np.array([-0.0905, 0.0, 0.9959]))) > 0.999

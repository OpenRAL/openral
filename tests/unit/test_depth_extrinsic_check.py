...
under test is a committed ``robots/<id>/robot.yaml`` with the sensor's pose swapped: the robot
manifest is the only place a robot sensor's mount lives.
...
def _verify(robot: Path, report: Path, sensor: str = "head_zed") -> int:
    return zc.main(["verify", "--robot", str(robot), "--sensor", sensor, "--report", str(report)])

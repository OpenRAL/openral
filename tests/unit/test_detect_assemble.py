...
def _report_with_two_cams() -> DetectionReport:
...
class TestEnrichCamerasFlag:
    """``enrich_cameras=False`` suppresses auto-append so the interactive
    sensor wizard (a later task) owns the sensor list instead."""

    def test_enrich_cameras_false_keeps_canonical_sensor_list(self) -> None:
        from openral_detect.registry import canonical_robot_path

        base = RobotDescription.from_yaml(str(canonical_robot_path("so101")))
        canonical_names = [s.name for s in base.sensors]

        desc = assemble_robot_description(
            _report_with_two_cams(), force_robot_type="so101", enrich_cameras=False
        )
        assert [s.name for s in desc.sensors] == canonical_names  # no camera_0/camera_1

    def test_enrich_cameras_true_appends_detected_cams(self) -> None:
        desc = assemble_robot_description(
            _report_with_two_cams(), force_robot_type="so101", enrich_cameras=True
        )
        names = [s.name for s in desc.sensors]
        assert "camera_0" in names and "camera_1" in names  # legacy behaviour intact

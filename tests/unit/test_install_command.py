...
class TestInstallGroupRegistry:
    ...
    def test_groups_mirror_workspace_root_pyproject(self) -> None:
        ...
        def _expand(name: str, _seen: set[str] | None = None) -> list[str]:
            ...
            _seen.add(name)
            result: list[str] = []
            for entry in ws_groups.get(name, []):
        ...
        for group_name, installer_pkgs in _GROUPS.items():
            ...
            # Include-group references (PEP 735) are expanded transitively so
            # the flat string lists are comparable.
            expanded = _expand(group_name)
            assert set(installer_pkgs) == set(expanded), (

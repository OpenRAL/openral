...
def _gpu_present() -> bool:
    ...
    ``nvidia-smi`` on PATH proves the driver is installed, not that the torch
    wheel was built with CUDA — a CPU-only torch would die mid-test instead of
    skipping (CLAUDE.md §1.11). Mirrors the guard in ``test_release_torch_modules.py``.
    """

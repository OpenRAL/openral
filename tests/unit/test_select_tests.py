...
def test_aloha_sim_test_selects_sim_dependency_lane() -> None:
    target = "tests/sim/test_aloha_bimanual_act_aloha.py"
    result = select_tests.select(REPO_ROOT, [target], CONFIG)
    assert target in result.targets
    assert result.requirement_targets["sim"] == [target]


def test_aloha_rskill_change_selects_sim_lane() -> None:
    target = "tests/sim/test_aloha_bimanual_act_aloha.py"
    result = select_tests.select(REPO_ROOT, ["rskills/act-aloha/rskill.yaml"], CONFIG)
    assert "tests/unit" in result.targets
    assert target in result.targets
    assert result.requirement_targets["sim"] == [target]
...
def test_lowering_requirement_group_is_reported() -> None:
    target = "tests/unit/test_collision_cli.py"
    result = select_tests.select(REPO_ROOT, [target], CONFIG)
    assert target in result.targets
    assert result.requirement_targets["lowering"] == [target]


def test_sidecar_wire_requirement_group_is_reported() -> None:
    target = "tests/unit/test_isaac_sim_sidecar_wire.py"
    result = select_tests.select(REPO_ROOT, [target], CONFIG)
    assert target in result.targets
    assert result.requirement_targets["sidecar-wire"] == [target]


def test_dataset_requirement_group_is_reported() -> None:
    target = "tests/unit/test_cli_dataset_from_bag.py"
    result = select_tests.select(REPO_ROOT, [target], CONFIG)
    assert target in result.isolated_targets
    assert result.requirement_targets["dataset"] == [target]

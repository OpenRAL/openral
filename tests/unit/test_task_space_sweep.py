...
# Pairs NOT executable by the DEFAULT-sim SimAttachedHAL OSC packers
# (SIM_EXECUTABLE_CONTROL_MODES) — both run sim through a dedicated controller
# path instead, so this is not a bug:
#   * 3d-diffuser-actor-rlbench — absolute CARTESIAN_POSE (pos+quat) consumed by
#     the RLBench/CoppeliaSim (PyRep) sidecar. Gripper EE name fixed:
#     panda_gripper -> panda_hand.
#   * rldx1-ft-gr1-nf4 — 29-D GR1 whole-body action (3 waist + 7+7 arms + 6+6
#     Fourier-hand fingers). Robot enumerates 17 joints; the 12 hand DoF are
#     EE-owned (two dexterous_hand EEs, n_dof=6 each), not joints, so the bare
#     29-wide joint segment exceeds 17. Runs via the robosuite BASIC composite
#     controller (gr1_unified wrapper). DOF-aware accounting (counting
#     EE-owned hand DoF toward the joint budget) is a follow-up.
#
...
def test_gr1_empty_modes_bug_fixed() -> None:
    """gr1 robot advertises joint_position (was empty) and its two Fourier hands as 6-DoF EEs.

    The 29-vs-17 DOF gap remains a recorded KNOWN_SIM_GAP (hand DoF is
    EE-owned); this pins that the empty-supported_control_modes bug, which
    dropped gr1 from the reasoner palette, is fixed.
    """
...
def test_rc365_sim_executable_after_fix() -> None:
    """rc365 cartesian slot EE fixed (panda_hand -> panda_gripper); now sim-executable."""

...
def refine_dop_to_budget(points: Points, dop_lo: Points, dop_hi: Points, budget: int) -> Points:
    ...
    budget, so it shipped **stage-1 only** — the 26-DOP, whose support gap
    against the real mesh is a median 4.52 mm and up to 25.68 mm. That is not
    academic: the 2026-09-07 battery's two ``panda_link1`` start-state stops sat
    3.86 mm and 8.68 mm beyond the voxel term, both inside that range, and the
    start-state census's deficit table says 10 mm of recovered clearance clears
    14 of 14 ``link1`` states.

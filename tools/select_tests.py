...
class SelectionResult(BaseModel):
    ...
    requirement_targets: dict[str, list[str]] = Field(default_factory=dict)
...
def _targets_matching_globs(repo_root: Path, globs: list[str]) -> set[str]:
    matched: set[str] = set()
    for glob in globs:
        for path in repo_root.glob(glob):
            if path.is_file():
...
def _requirement_targets(
    ...
    *,
    ...
    for requirement, globs in config.requirement_globs.items():
        hits = sorted(
            target for target in selected if any(fnmatch.fnmatch(target, glob) for glob in globs)
        )
        if hits:
            out[requirement] = hits
    return out
...
def select(
    ...
    return SelectionResult(
        ...
        requirement_targets=_requirement_targets(
            repo_root, config, targets, isolated, full_run=False
        ),
...
def _render_human(result: SelectionResult) -> str:
    ...
    if result.isolated_targets:
        ...
        for tgt in result.isolated_targets:
            ...
            for reason in result.reasons.get(tgt, []):
                lines.append(f"      ← {reason}")
    if result.requirement_targets:
        lines.append("")
        lines.append("Opt-in dependency lanes:")
        for requirement, targets in sorted(result.requirement_targets.items()):
            lines.append(f"  • {requirement}: {' '.join(targets)}")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.github_output:
        ...
        payload = {
            ...
            "requirement_targets_json": json.dumps(result.requirement_targets),
            "any": (
                "true"
